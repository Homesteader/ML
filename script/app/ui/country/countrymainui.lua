...
function CountryMainUI:init()
    ...
    back_btn:setPosition(cc.p(winSize.width - 35, winSize.height - 45))
    ...
    local top_img = self.root:getChildByName("top_img")
    top_img:setPosition(cc.p(winSize.width/2, winSize.height - 65))
...
function CountryMainUI:initDown(winSize, country)
    ...
    local startX = (1140 - winSize.width)/2 + 80

    self.light_img = down_img:getChildByName("light_img")
    self.light_img:ignoreContentAdaptWithSize(true)
    self.light_img:loadTexture("uires/ui/country/flag_" .. country .. "_" .. country .. ".png")
    self.light_img:setVisible(false)
    ...
    flag_img:loadTexture("uires/ui/country/flag_" .. country .. ".png")
...
function CountryMainUI:initPlayerInfo(winSize)
    ...
    local lv_bg_img = player_info_node:getChildByName("lv_bg_img")
    local size = lv_bg_img:getContentSize()
    ...
    self.player_lv_tx:setPosition(cc.p(size.width/2,size.height/2))
    lv_bg_img:addChild(self.player_lv_tx)

    self.player_vip_img = player_info_node:getChildByName("vip_img")
    self.player_progress = cc.ProgressTimer:create(cc.Sprite:create('uires/ui/buoy/bar.png'))
    self.player_progress:setType(cc.PROGRESS_TIMER_TYPE_RADIAL)
    self.player_progress:setPosition(cc.p(54,49))
    ...
    self.player_vip_tx:setPosition(cc.p(size.width/2 + 25, size.height/2 - 18))
...
function CountryMainUI:updateInfo()
    ...
    fightForce_tx:setPosition(cc.p(105,-5))
...
function CountryMainUI:updateMiddle()
    ...
    local heroBaseConf,heroCombatConf,heroModelConf = GlobalApi:getHeroConf(tonumber(self.list[1][4]))
    if self.kingAni == nil then
        self.kingUrl = heroModelConf.modelurl
        local dizuo_img = self.king_bg_img:getChildByName("dizuo_img")
        local posx, posy = dizuo_img:getPosition()
        local promote = nil
        ...
        self.kingAni = GlobalApi:createLittleLossyAniByName(heroModelConf.modelUrl .. "_display", nil, changeEquipObj)
        self.kingAni:getAnimation():play("idle", -1, -1)
        self.kingAni:setPosition(cc.p(posx, posy + heroModelConf.uiOffsetY + 20))
        self.king_bg_img:addChild(self.kingAni)
        local shadow = self.kingAni:getBone(heroModelConf.modelUrl .. "_display_shadow")
        if shadow then
            shadow:changeDisplayWithIndex(-1, true)
        end
    elseif self.kingUrl ~= heroModelConf.modelUrl then
        self.kingUrl = heroModelConf.modelUrl
