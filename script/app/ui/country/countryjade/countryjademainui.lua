...
function CountryJadeMainUI:init()
    ...
    self.jadeCount = self.root:getChildByName("jade_count")
    self.jadeMineFightTx = self.root:getChildByName("jade_mine_fight_tx")
    self.jadeBg = self.root:getChildByName("fight_bg")

    self.jadeJadeTx = self.root:getChildByName("mine_jade_tx")
    self.jadeJadeImg = self.root:getChildByName("mine_jade_img")
    self.jadeJadeTxWu = self.root:getChildByName("mine_jade_tx_wu")

    self.robBtn = self.root:getChildByName("rob_btn")
    self.reportBtn = self.root:getChildByName("report_btn")
    self.refreshBtn = self.root:getChildByName("refresh_btn")   -- 现在改为转化按钮
    self.refreshBtn:loadTextureNormal("uires/ui/country/jade/zhuanhuan.png")
    self.getJadeBtn = self.root:getChildByName("get_jade_btn")
    ...
    self:adaptUI(bgimg, bgimg1,true)
    self:adaptUI2()
...
function CountryJadeMainUI:adaptUI2()
    ...
    self.jadeCount:setPosition(cc.p(30,winSize.height - 40 - 40))
    self.jadeMineFightTx:setPosition(cc.p(30,winSize.height - 40))
    self.jadeBg:setPosition(cc.p(130.64,winSize.height - 40))

    self.jadeJadeTx:setAnchorPoint(cc.p(0,0.5))
    self.jadeJadeTx:setString(GlobalApi:getLocalStr('COUNTRY_JADE_DES11'))
    self.jadeJadeTx:setPosition(cc.p(30,winSize.height - 35))
    self.jadeJadeImg:setPosition(cc.p(30 + self.jadeJadeTx:getContentSize().width + 30,winSize.height - 35))
    self.jadeJadeTxWu:setPosition(cc.p(30 + self.jadeJadeTx:getContentSize().width + 5,winSize.height - 35))

    local offset = 100
    self.robBtn:setPosition(cc.p(30,55))
    self.reportBtn:setPosition(cc.p(30 + offset,55))
    self.refreshBtn:setPosition(cc.p(30 + offset * 3,55))
    self.getJadeBtn:setPosition(cc.p(30 + offset * 2,55))
    self.backBtn:setPosition(cc.p(30,55))

    self.arrowImg:setPosition(cc.p(winSize.width - (960 - 889),58))

    local btn = HelpMgr:getBtn(HELP_SHOW_TYPE.COUNTRY_JADE)
    btn:setPosition(cc.p(winSize.width - 110,winSize.height - 40))
...
function CountryJadeMainUI:updateMyCountryCorner()
    ...
    self.jadeMineFightTx:setVisible(false)
    self.jadeBg:setVisible(false)
...
function CountryJadeMainUI:updateOtherCountryCorner()
    ...
    self.jadeMineFightTx:setVisible(true)
    self.jadeBg:setVisible(true)
    ...
    -- 战力
    if not self.fightforceLabel then
        ...
        fightforceLabel:setPosition(cc.p(self.jadeBg:getPositionX() + 5,self.jadeBg:getPositionY()))
        self.root:addChild(fightforceLabel)
        self.fightforceLabel = fightforceLabel
    end
    self.fightforceLabel:setString(RoleData:getFightForce())
end
    ...
    for i = 1,2 do  -- 1带表左，2带表右
        ...
        if data.uid == 0 then
            ...
            if data.fight_force > 10000 then
                ...
                if i == 1 then
                    fightforceLabel:setAnchorPoint(cc.p(1, 0.5))
                    fightforceLabel:setPosition(cc.p(fight_wan:getPositionX() - fightforceLabelOffset,fight_wan:getPositionY()))
                    fight_bg:setPositionX(fightforceLabel:getPositionX() - fightforceLabelWidth - fightforceLabelOffset)
                else
                    fightforceLabel:setAnchorPoint(cc.p(0, 0.5))
                    fightforceLabel:setPosition(cc.p(fight_bg:getPositionX() + fightforceLabelOffset,fight_wan:getPositionY()))
                    fight_wan:setPositionX(fightforceLabel:getPositionX() + fightforceLabelWidth + fightforceLabelOffset)
                end
                ...
                if i == 1 then
                    fightforceLabel:setAnchorPoint(cc.p(1, 0.5))
                    fightforceLabel:setPosition(cc.p(name:getPositionX(),fight_wan:getPositionY()))
                    fight_bg:setPositionX(fightforceLabel:getPositionX() - fightforceLabelWidth - fightforceLabelOffset)
                else
                    fightforceLabel:setAnchorPoint(cc.p(0, 0.5))
                    fightforceLabel:setPosition(cc.p(fight_bg:getPositionX() + fightforceLabelOffset,fight_wan:getPositionY()))
