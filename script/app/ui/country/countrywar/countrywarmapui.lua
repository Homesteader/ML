    ...
    for i,v in ipairs(leftIds) do
        for j=v,rightIds[i] do
            ...
            if j >= 1 and j <= 35 then
                ...
                if not bgImg then
                    cc.Director:getInstance():getTextureCache():addImageAsync('uires/ui/countrywar/bg_'..j..'.jpg',function(texture)
                        ...
                        local img = ccui.ImageView:create('uires/ui/countrywar/bg_'..j..'.jpg')
...
function CountryWarMapUI:createNameBg(btn,cityId)
    ...
    local nameImg = ccui.ImageView:create('uires/ui/countrywar/name_bg_'..cityData.hold_camp..'.png')
    local nameTx = cc.Label:createWithTTF("", "font/gamefont1.TTF", 20)
...
function CountryWarMapUI:updateCityBtnCell(cityId)
    ...
    if btn and not nameBgImg then
        ...
        if nameBgImg then
            nameBgImg:loadTexture('uires/ui/countrywar/name_bg_'..cityData.hold_camp..'.png')
        end
    end
    ...
    local functions = {
        ...
        ['forward'] = function()
            ...
            if cityData.fight_state == 1 and CountryWarMgr.myCity ~= index then
                ...
                local time = GlobalData:getServerTime()
...
function CountryWarMapUI:initLoading()
    ...
    cc.Director:getInstance():getTextureCache():addImageAsync('uires/ui/countrywar/bg_img.jpg',imageLoaded)
end
...
function CountryWarMapUI:updateSmallMapCity()
    ...
    if correctTime >= tonumber(CountryWarMgr:getBaseValue('readyTime')) + CountryWarMgr.openTimeBeginTime then
        flagImg:loadTexture('uires/ui/countrywar/flag_small_'..CountryWarMgr.camp..'.png')
