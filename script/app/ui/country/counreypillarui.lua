...
function CountryPillarUI:init()
    ...
    local close_btn = scroll_img_2:getChildByName("close_btn")
    ...
    for i = 1, totalNum do
		...
		local heroBaseConf,heroCombatConf,heroModelConf = GlobalApi:getHeroConf(tonumber(self.data[i][4]))
        ...
        local shadow = hero:getBone(heroModelConf.modelUrl .. "_display_shadow")
        if shadow then
            shadow:changeDisplayWithIndex(-1, true)
        end
        hero:setPosition(cc.p(widgetW/2, heroModelConf.uiOffsetY))
        widget:addChild(hero)

        local position_tx = ccui.Text:create()
        position_tx:setFontName("font/gamefont1.TTF")
        ...
        if i == 1 then
            ...
            position_tx:setString(positionConf[i].title)
        end
