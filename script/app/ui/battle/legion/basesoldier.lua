...
function BaseSoldier:useSkill(skillObj)
	...
	skillObj:useSkill()
	if self.talentSkill then
		self.talentSkill:effectWhenUseSkill()
	end
...
function BaseSoldier:getEffect(atker, hpNum, showNum, flag, skillType, damageCoefficient, addAnger)
	...
	self:recordDmgInReport(atker, hpNum, showNum, flag, skillType)
...
function BaseSoldier:effectWhenHurtTarget(hpNum, flag, target)
	if self.talentSkill then
		return self.talentSkill:effectWhenHurtTarget(hpNum, flag, target)
	else
		return hpNum
	end
end

function BaseSoldier:effectWhenGetHurt(hpNum, flag, target)
	if self.talentSkill then
		return self.talentSkill:effectWhenGetHurt(hpNum, flag, target)
	else
		return hpNum
	end
end
...
function BaseSoldier:playKillAnimation(x, y)
	local killSprite = BattleHelper:createLittleLossyAniByName("battle_kill")
	killSprite:setScale(5.0)
	killSprite:setPosition(x, y + 80)
	self.battlefield.bgImg:addChild(killSprite)
	BattleHelper:setZorder(killSprite, y + 80, 2)
	killSprite:runAction(cc.Sequence:create(cc.ScaleTo:create(0.32, 1),
							cc.CallFunc:create(function ()
								killSprite:getAnimation():playWithIndex(0, -1, -1)
							end),
							cc.DelayTime:create(0.7),
							cc.FadeOut:create(0.3),
							cc.CallFunc:create(function ()
								killSprite:removeFromParent()
							end)))
end
...
function BaseSoldier:recordDmgInReport(atker, hpNum, showNum, flag, skillType)
end
