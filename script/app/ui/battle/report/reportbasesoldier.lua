...
function ReportBaseSoldier:recordDmgInReport(atker, hpNum, showNum, flag, skillType)
	local skillId = 0
	if skillType == SKILL_TYPE.NORMAL_ATTACK then
		skillId = atker.baseSkill.skillId
	elseif skillType == SKILL_TYPE.AUTO_SKILL then
		skillId = atker.autoSkills.skillId
	elseif skillType == SKILL_TYPE.ANGER_SKILL then
		skillId = atker.angerSkill.skillId
	end
	local obj = {
		[1] = self.battlefield.time,
		[2] = REPORT_NAME.HURT,
