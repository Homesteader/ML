...
local function getPlayerArmyData(attrs, mercenary, slots)
	...
	for _, i in ipairs(slots) do
		...
		if heroObj and heroObj:getId() > 0 then
			...
			local soldierStar = heroObj:getSoldier().star    
			local soldierLvConf = GameData:getConfData("soldierlevel")[soldierId][soldierLv][soldierStar]
			army.soldierInfo,army.soldierModelInfo = GlobalApi:getSoldierConf(soldierLvConf.soldierId) --军队信息
			local heroBaseInfo,heroCombatInfo,heroModelInfo = GlobalApi:getHeroConf(heroObj:getId())

			-- 突破相关
			local talentSkill
			local talentLevel = heroObj:getTalent()
			if talentLevel > 0 and heroCombatInfo.innateGroup > 0 then
				local haveTalent = false
				local talent = {
					skill1 = false,
					skill2 = false
				}
				local innategroupConf = GameData:getConfData("innategroup")[heroCombatInfo.innateGroup]
			...
			local legionInfo = {
				...
				rolePos = i,
				soldierUrl = army.soldierModelInfo.modelUrl,
				soldierNum = soldierLvConf.num,
				soldierLv = soldierLv,
				headpic = "uires/icon/hero/" .. heroModelInfo.headIcon,
				isMonster = false
			}
			local hero = {
				...
				moveSpeed = heroCombatInfo.baseSpeed*(1 + heroAtts[attrIndex.MOVESPEEDPERCENT]/100),
				...
				attackRange = heroCombatInfo.attackRange,
				bodySize = heroCombatInfo.bodySize,
				attackType = heroCombatInfo.attackType,
				professionType = heroCombatInfo.professionType,
				promote = heroObj:getPromoted(),
				changeEquipObj = heroObj:getChangeEquipState(),
				skillGroupId = heroCombatInfo.skillGroupId[heroObj:getPromoteType()],
				skillLevel = heroObj:getDestiny().level,
				legionType = heroCombatInfo.legionType,

				url = heroObj:getUrl(),
				scale = heroModelInfo.scale,
				hpBarHeight = heroModelInfo.hpBarHeight,
				attPx_t = heroModelInfo.attPx_t,
				attPy_t = heroModelInfo.attPy_t,
				attPx_c = heroModelInfo.attPx_c,
				attPy_c = heroModelInfo.attPy_c,
				attPx_b = heroModelInfo.attPx_b,
				attPy_b = heroModelInfo.attPy_b,
				hitScale = heroModelInfo.hitScale,
				buffScale = heroModelInfo.buffScale,
				deadEffect = heroModelInfo.deadEffect,
				talentSkill = talentSkill,
				camp = heroCombatInfo.camp
			}
...
local function createEnemyDataByServer(info, healths)
	...
	for k, v in pairs(info.pos) do
		if v.attr[tostring(attrIndex.HP)] > 0 then
			...
			local heroBaseInfo,heroCombatInfo,heroModelInfo = GlobalApi:getHeroConf(tonumber(v.hid))
			local soldierId = heroBaseInfo.soldierId
			local soldierLv = v.soldier_level
			local soldierStar = v.soldier_star    
			release_print('soldierId = ' .. soldierId .. ', soldierLv = ' .. soldierLv .. ', soldierStar = ' .. soldierStar)
			local soldierLvConf = GameData:getConfData("soldierlevel")[soldierId][soldierLv][soldierStar]
			enemy.soldierInfo,enemy.soldierModelInfo = GlobalApi:getSoldierConf(soldierLvConf.soldierId) --军队信息
			-- 突破相关
			local talentSkill
			...
			local legionInfo = {
				pos = tonumber(v.slot),
				rolePos = tonumber(k),
				soldierUrl = enemy.soldierModelInfo.modelUrl,
	        ...
	        if v.promote and v.promote[1] then
                ...
                if promoteProgress > #heroCombatInfo.skillGroupId then
                    promoteProgress = #heroCombatInfo.skillGroupId
                end
			end
	        if heroCombatInfo.camp == 5 then
			...
			local hero = {
				...
				skillGroupId = heroCombatInfo.skillGroupId[promoteProgress],
				skillLevel = v.destiny,
				legionType = heroCombatInfo.legionType,

				url = heroModelInfo.modelUrl,
				scale = heroModelInfo.scale,
				hpBarHeight = heroModelInfo.hpBarHeight,
				attPx_t = heroModelInfo.attPx_t,
				attPy_t = heroModelInfo.attPy_t,
...
function BattleMgr:createEnemyDataByMonsterId(ids)
	...
	local monsterIdArr = {}
	for i = 1, 9 do
		local heroBaseConf = GlobalApi:getHeroConf(ids[i])
		if ids[i] and ids[i] > 0 then
			monsterIdArr[i] = heroBaseConf.trialMonsterId
		else
			monsterIdArr[i] = 0
		end
	end
...
function BattleMgr:createEnemyData(enemyData, monsterIdArr, boss, healths)
	...
	for i = 1, 9 do
		...
		if monsterId > 0 then
			index = index + 1
			local enemy = {}
			local monsterObj,monsterCombatConf,monsterModelConf = GlobalApi:getMonsterConf(monsterId)
			-- 突破相关
			local talentSkill
			local talentLevel = monsterObj.talentLevel
			if talentLevel > 0 and monsterCombatConf.innateGroup > 0 then
				local haveTalent = false
				local talent = {
					skill1 = false,
					skill2 = false
				}
				local innategroupConf = GameData:getConfData("innategroup")[monsterCombatConf.innateGroup]
				for i = 1, talentLevel do
					if innategroupConf["level" .. i] > 1000 then
						haveTalent = true
						local specialId = innategroupConf["level" .. i]-1000
			...
			local legionInfo = {
				...
				headpic = "uires/icon/hero/" .. monsterModelConf.headIcon,
				isMonster = true
			}
			if monsterObj.soldierId > 0 then
				enemy.soldierInfo,enemy.soldierModelInfo = GlobalApi:getSoldierConf(monsterObj.soldierId) --军队信息
				legionInfo.soldierUrl = enemy.soldierModelInfo.modelUrl
			...
			if monsterHp > 0 then
				local monster = {
					...
					heroQuality = monsterCombatConf.quality,
					camp 		= monsterCombatConf.camp,
					moveSpeed 	= monsterCombatConf.baseSpeed,
					attackRange = monsterCombatConf.attackRange,
					pvpAttPercent = 0,
					pvpDefPercent = 0,
					bodySize 	= monsterCombatConf.bodySize,
					attackType 	= monsterCombatConf.attackType,
					baseAttackSpeed = monsterCombatConf.attackSpeed,
					attackSpeed = monsterCombatConf.attackSpeed,
					professionType = monsterCombatConf.professionType,
					promote 	= {},
					skillGroupId = monsterCombatConf.skillGroupId[1],
					legionType 	= monsterCombatConf.legionType,

					url 		= monsterModelConf.modelUrl,
					scale 		= monsterModelConf.scale,
					hpBarHeight = monsterModelConf.hpBarHeight,
					attPx_t 	= monsterModelConf.attPx_t,
					attPy_t 	= monsterModelConf.attPy_t,
					attPx_c 	= monsterModelConf.attPx_c,
					attPy_c 	= monsterModelConf.attPy_c,
					attPx_b 	= monsterModelConf.attPx_b,
					attPy_b 	= monsterModelConf.attPy_b,
					hitScale 	= monsterModelConf.hitScale,
					buffScale 	= monsterModelConf.buffScale,
...
function BattleMgr:playBattle(battleType, customObj, returnFunc)
	...
	if battleType == BATTLE_TYPE.NORMAL then -- 普通
		local customsId = customObj.id
		local customsCfg = GameData:getConfData("custom")[customsId]
		local monsterGroup = customsCfg['formation'][customObj.customsType]
	...
	elseif battleType == BATTLE_TYPE.TOWER then -- 爬塔
        ...
        armyData = getPlayerArmyData(nil, nil, nil)
    ...
    elseif  battleType == BATTLE_TYPE.TERRITORALWAR_PUPPET then
		...
		enemyData.legionArr = newLegionArr
		enemyData.fightforce = fightforce
	elseif battleType == BATTLE_TYPE.HOOK_MONSTER then
		enemyData = self:createEnemyDataByConf(customObj.formationId)
	end
...
function BattleMgr:showBattleReportResult()
	if self.uiClass["reportResultUI"] == nil then
		self.uiClass["reportResultUI"] = ClassBattleReportResultUI.new()
...
function BattleMgr:showBattleResult(isVictory, displayAwards, starNum,specialAwardDatas,kingLvUpData)
	if isVictory then
...
function BattleMgr:showBattleResultWithoutBattlefield(isVictory, displayAwards, starNum, specialAwardDatas, kingLvUpData, damageInfo)
	if isVictory then
		if self.uiClass["victoryUI"] == nil then
			self.uiClass["victoryUI"] = ClassBattleVictoryUI.new(displayAwards, starNum, specialAwardDatas, kingLvUpData, true, damageInfo)
			self.uiClass["victoryUI"]:showUI()
			AudioMgr.playEffect("media/effect/battle_victory.mp3", false)
		end
	else
...
function BattleMgr:calculateReport(battleType, customObj, callback)
	...
	elseif battleType == BATTLE_TYPE.NEW_LEGION_TRIAL then --新版军团开黑
		...
		local mercenary = createEnemyDataByServer(obj)
		armyData = getPlayerArmyData(nil, mercenary, nil)
	elseif battleType == BATTLE_TYPE.TRIAL then --秘境矿洞
        local obj = {
			name = "",
			pos = customObj.trial_robot,
            index = customObj.index
