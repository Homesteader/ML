...
function BattleReplayUI:createLegion(guid, data)
	...
	for k, v in ipairs(skillArr) do
		if v.summonId > 0 then
			...
			local baseInfo,summonModelInfo = GlobalApi:getSoldierConf(summonInfo.modelArmyTypeId)
			if guid == 1 then
				self.reportField:getSoldierActionInfo(summonModelInfo.modelUrl .. "_g")
			else
				self.reportField:getSoldierActionInfo(summonModelInfo.modelUrl .. "_r")
			end
	...
	return ClassReplayLegion.new(guid, data, self)
end
	...
	for i = 1, 2 do
		for k, v in ipairs(self.playerSkillArr[i]) do
			if v.info.res ~= "0" then
				self.reportField:getDragonActionInfo(v.info.res, "animation_littlelossy/" .. v.info.res .. "/" .. v.info.res .. ".json")
			end
...
function BattleReplayUI:sendMessageAfterFight(isWin)
    ...
    elseif self.battleType == BATTLE_TYPE.TERRITORALWAR_PLAYER or
		...
		BattleMgr:showBattleReportResult()
	end
end
...
function BattleReplayUI:showBattleStartAnimation()
	...
	local act = cc.Sequence:create(cc.DelayTime:create(0.5), 
				cc.CallFunc:create(function ()
					...
					startAni:setAnimation(0, "idle1", false)
