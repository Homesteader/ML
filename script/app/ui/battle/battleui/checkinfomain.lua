...
function CheckInfoMainUI:initTop()
    ...
    if self.data.info.notrealuser and self.data.info.notrealuser == true then
        ...
        if self.data.info.legion_name == "" then
            ...
            if self.data.info.legion_icon then
                ...
                legionImg:loadTexture('uires/ui/legion/' .. self.data.info.legion_icon .. '_jun.png')
	...
	for k,v in pairs(self.data.info.pos) do
		...
		if heroBaseConf~=nil then
			...
			hero.name=(hero.isKing==true) and GlobalApi:getLocalStr('STR_MAIN_NAME') or heroBaseConf.heroName
...
function CheckInfoMainUI:initBottom()
    ...
    for i = 1,4 do
        ...
        elseif i == 3 then
            if type(self.data.info.arenaRank) == 'table' then
                nameTx:setString(tostring(self.data.info.arenaRank.rank))
                if self.data.info.arenaRank.rank == 0 then
                    nameTx:setString(GlobalApi:getLocalStr("RANKING_NO_INLIST"))
                end
            else
                nameTx:setString(tostring(self.data.info.arenaRank))
