...
function EmbattleUI:init()
            ...
            skill_border:addClickEventListener(function ()
                MainSceneMgr:showSkillSelect(function(id)
                    if skills[i] ~= id then
                        ...
                        skillBorderArr[i].icon:loadTexture("uires/ui/treasure/" .. playerSkillConf[id].icon)
    ...
    for k, v in pairs(self.data.skills) do
        ...
        if tonumber(v) > 0 then
            ...
            skillBorderArr[tonumber(k)].icon:loadTexture("uires/ui/treasure/" .. playerSkillConf[tonumber(v)].icon)
    ...
    for i = 1, 7 do
        ...
        if standPos and standPos > 0 then
            ...
            if heroObj and heroObj:getId() > 0 then
                local heroBaseInfo,heroCombatConf,heroModelConf = GlobalApi:getHeroConf(heroObj:getId())
                local ani = GlobalApi:createAniByName(heroObj:getUrl(), nil, heroObj:getChangeEquipState())
                bg_img:addChild(ani)
                ani:getAnimation():play("idle", -1, -1)
                ani:setScale(-heroModelConf.scale/100, heroModelConf.scale/100)
                aniMap[standPos] = {}
                aniMap[standPos].hero = ani
                aniMap[standPos].legionType = heroCombatConf.legionType
                aniMap[standPos].slot = i
                ani:setPosition(cc.p(LEGION_POS[standPos]))
                slotMap[i] = 1
                team[tostring(i)] = standPos
                armyflags[standPos]:setVisible(true)
                armyflags[standPos]:loadTexture("uires/ui/battle/flag_" .. heroCombatConf.legionType .. ".png")
            end
        end
    end
    -- 剩下的没有位置的武将找空位站
    ...
    for i = 1, 7 do
        ...
        if heroObj and heroObj:getId() > 0 then
            if slotMap[i] == nil then
                ...
                aniMap[posIndex].legionType = heroCombatConf.legionType
                aniMap[posIndex].slot = i
                ani:setPosition(cc.p(LEGION_POS[posIndex]))
                slotMap[i] = 1
                team[tostring(i)] = posIndex
                armyflags[posIndex]:setVisible(true)
                armyflags[posIndex]:loadTexture("uires/ui/battle/flag_" .. heroCombatConf.legionType .. ".png")
                posIndex = posIndex + 1
            end
        end
    end
