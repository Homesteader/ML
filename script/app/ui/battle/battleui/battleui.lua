...
local FONT_FILE = {"uires/ui/number/font1_negative.fnt", "uires/ui/number/font1_yellownum.fnt", "uires/ui/number/font1_positive.fnt", "uires/ui/number/font1_cure_crit.fnt"}
local FONT_NUM = {100, 100, 50, 10}
local WINSIZE = cc.Director:getInstance():getVisibleSize()
local SPEEDUP_SCALE = {1,1.5,2}
local STR_MISS = "闪避"
local STR_IMMUNE = "免疫"
...
function BattleUI:ctor(battleType, armyData, enemyData, customObj)
	...
	elseif playerLv < speedUpLv2 then
		self.speedUp = cc.UserDefault:getInstance():getFloatForKey(uid .. "_battle_speed", 1.0)
...
function BattleUI:loadResource()
	...
	local function _loadSkillRes(guid, skillInfo)
		...
		if skillInfo.summonId > 0 then
			...
			local soldierInfo,soldierModelInfo = GlobalApi:getSoldierConf(summonInfo.modelArmyTypeId)
			local soldierUrl
			if guid == 1 then
				soldierUrl = "animation/" .. soldierModelInfo.modelUrl .. "_g/" .. soldierModelInfo.modelUrl .. "_g"
			else
				soldierUrl = "animation/" .. soldierModelInfo.modelUrl .. "_r/" .. soldierModelInfo.modelUrl .. "_r"
			end
	...
	local plistArr = {"uires/ui/battle/plist/battle_ui", "animation_littlelossy/xiaobing/xiaobing", "animation/battle_bullet/battle_bullet"}
...
function BattleUI:initField()
	...
	for i = 1, 3 do
		...
		battleCounterAni:setPosition(cc.p((LEGION_POS[2][i].x + LEGION_POS[1][i].x)/2, LEGION_POS[1][i].y))
						...
						local swopAction = cc.Sequence:create(cc.MoveTo:create(0.2, LEGION_POS[1][i]), cc.CallFunc:create(function ()
							...
							for j = 1, 5 do
								if swopLegion.soldierLv > 1 and starNum >= j then
									...
									self.armyStars[i][j]:setTexture("uires/ui/role/star_" .. starLv .. ".png")
								else
					...
					action = cc.Sequence:create(cc.MoveTo:create(0.2, LEGION_POS[1][minIndex]), cc.CallFunc:create(function ()
						...
						for j = 1, 5 do
							if targetLegion.soldierLv > 1 and starNum >= j then
								...
								self.armyStars[minIndex][j]:setTexture("uires/ui/role/star_" .. starLv .. ".png")
							else
...
function BattleUI:initSkillListPanel()
	...
	for i = 1, 5 do
		...
		if playerskill then
			...
			dragon:setTexture("uires/ui/treasure/" .. playerskill.info.icon)
						...
						if touchBegin then
							...
							if currDragon.visible then
								if currPos.y <= skillBorderPos[1].y + 100 then
									...
									currDragon.dragon:setPosition(cc.p(currPos.x + currDragon.offsetX, currPos.y + currDragon.offsetY))
...
function BattleUI:initCornerUI()
	...
	self.backBtn2:setPosition(cc.p(WINSIZE.width - 60, WINSIZE.height - 60))
	...
	local backBtn = pauseBg:getChildByName("back_btn")
	backBtn:setPosition(cc.p(WINSIZE.width*0.5 - 100, WINSIZE.height*0.5))
	...
	local backLabel = pauseBg:getChildByName("back_text")
	backLabel:setPosition(cc.p(WINSIZE.width*0.5 - 100, WINSIZE.height*0.5 - 60))
	backLabel:setString(GlobalApi:getLocalStr("STR_QUIT"))

	local continueBtn = pauseBg:getChildByName("continue_btn")
	continueBtn:setPosition(cc.p(WINSIZE.width*0.5 + 100, WINSIZE.height*0.5))
	...
	local continueLabel = pauseBg:getChildByName("continue_text")
	continueLabel:setPosition(cc.p(WINSIZE.width*0.5 + 100, WINSIZE.height*0.5 - 60))
	...
	self.pauseBtn:setPosition(cc.p(60, WINSIZE.height - 60))
	...
	self.speedBtn:setPosition(cc.p(130, WINSIZE.height - 60))
...
function BattleUI:initLegionInfoImg()
	...
	for i = 1, 9 do
		...
		for j = 1, 5 do
			...
			if starNum >= j then
				self.armyStars[9 + i][j]:setTexture("uires/ui/role/star_" .. starLv .. ".png")
			else
...
function BattleUI:initLegion(callback)
	release_print('BattleUI:initLegion ####')
	...
	local co = coroutine.create(function ()
		...
		for k, v in ipairs(self.armyData.legionArr) do
			index = index + 1
			local legion = self:createLegion(self.armyData.guid, v)
		...
		for k2, v2 in ipairs(self.enemyData.legionArr) do
			index = index + 1
			local enemyLegion = self:createLegion(self.enemyData.guid, v2)
...
function BattleUI:createLegion(guid, data)
	return ClassBattleLegion.new(guid, data, self)
end
...
function BattleUI:sendMessageBeforeFight()
	...
	if self.battleType == BATTLE_TYPE.NORMAL 
		...
		local args = {
			type = self.customObj.customsType,
		...
		MessageMgr:sendPost("before_fight", "battle", json.encode(args), function (jsonObj)
			...
			if code == 0 then
				...
				promptmgr:showMessageBox(GlobalApi:getLocalStr_new("BATTLE_STR_ERROR"), MESSAGE_BOX_TYPE.MB_OK, function ()
					BattleMgr:exitBattleField()
				end)
			end
		end)
	...
	elseif 	self.battleType == BATTLE_TYPE.TRIAL then
		...
		local trailIndex = self.customObj.index

		LegionTrialMgr:legionTrialBeforeFightFromServer(trailIndex,function (jsonObj)
			local code = jsonObj.code
			if code == 0 then
				self.verifyObj.pos = jsonObj.data.rand_pos or 1
				self.verifyObj.attrs = jsonObj.data.rand_attrs or {}
	...
	elseif self.battleType == BATTLE_TYPE.HOOK_MONSTER then
		local args = {
		}
        UserData:getUserObj():setFormation(formation)
        MessageMgr:sendPost("before_fight", "auto_fight", json.encode(args), function (jsonObj)
        	local code = jsonObj.code
			if code == 0 then
				self:battleStart()
			else
...
function BattleUI:sendMessageAfterFight(isWin)
	if self.battleType == BATTLE_TYPE.NORMAL then
		...
		MessageMgr:sendPost("fight", "battle", json.encode(args), function (jsonObj)
			...
			if code == 0 then
				...
				if isWin then
					BattleMgr:setReturnObj(isWin)
					local mapObj = MapData.data[self.customObj.id]
					mapObj:addTimes(self.customObj.customsType, 1)
					mapObj:setPass(self.customObj.customsType)
					MapData:setMaxFightedCustoms(self.customObj.customsType,self.customObj.id)
				end
 ...
 --                kingLvUpData.nowLv = UserData:getUserObj():getLv()
	...
	elseif self.battleType == BATTLE_TYPE.HOOK_MONSTER then
		...
		MessageMgr:sendPost("fight", "auto_fight", json.encode(args), function (jsonObj)
			if jsonObj.code == 0 then
...
function BattleUI:showBattleStartAnimation()
	local startAni = GlobalApi:createSpineByName("zhandoukaishi", "spine/zhandoukaishi/zhandoukaishi", 1)
	startAni:setVisible(false)
	startAni:setScale(0.6)
	...
	local act = cc.Sequence:create(cc.DelayTime:create(0.5), 
				cc.CallFunc:create(function ()
					...
					startAni:setAnimation(0, "idle2", false)
...
function BattleUI:battleStart()
	...
	if self.canSpeedUp == 1 then
		...
		tipsTx:setFontName("font/gamefont1.TTF")
		...
		tipsTx:setPosition(cc.p(130, WINSIZE.height - 95))
...
function BattleUI:usePlayerSkill(guid, index, pos)
	...
	if effectInfo then
		if guid == 1 then
			...
			sprite1:setPosition(cc.p(-200, WINSIZE.height-150))
			...
			sprite1:runAction(cc.Sequence:create(cc.EaseOut:create(cc.MoveTo:create(0.8/TIMESCALE, cc.p(200, WINSIZE.height-150)), 20), cc.Spawn:create(cc.MoveTo:create(0.15/TIMESCALE, cc.p(WINSIZE.width/2, WINSIZE.height-150)), cc.FadeOut:create(0.15/TIMESCALE)), cc.CallFunc:create(function ()
...
-- 击杀效果
function BattleUI:addKillAnimation(dieGuid, legionPos, dieurl, killurl)
	...
	if killurl then
		if self.killAnimationFlag then
			...
			self:playKillAnimation(dieGuid, self.killHeadNum[dieGuid], killurl, dieurl)
		end
	else
...
function BattleUI:playKillAnimation(dieGuid, index, killurl, dieurl)
	...
	widget:runAction(cc.Sequence:create(cc.MoveTo:create(0.1, cc.p(0, WINSIZE.height/2 + 100)), cc.CallFunc:create(function ()
		killImg:setVisible(true)
		killImg:getAnimation():playWithIndex(0, -1, 0)
	...
	end), cc.DelayTime:create(0.5), cc.Spawn:create(cc.MoveBy:create(0.6, cc.p(0, 80)), cc.FadeOut:create(0.6)), cc.CallFunc:create(function ()
		widget:removeFromParent()
		if #self.killAnimationList > 0 then
			local obj = table.remove(self.killAnimationList)
			self:playKillAnimation(obj.dieGuid, obj.index, obj.killurl, obj.dieurl)
		else
			self.killAnimationFlag = false
		end
	end)))
...
function BattleUI:canUseTalentSkillBeforeFight(guid, heroId)
	if self.talentSkillBeforeFight[guid][heroId] then
		return false
	else
		self.talentSkillBeforeFight[guid][heroId] = true
		return true
	end
end
