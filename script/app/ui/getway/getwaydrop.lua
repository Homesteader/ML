local GetWayDropUI = class("GetWayDropUI", BaseUI)
local ClassItemCell = require('script/app/global/itemcell')

function GetWayDropUI:ctor(obj)
    self.uiIndex = GAME_UI.UI_GET_WAY_DROP
    self.obj = obj

end

function GetWayDropUI:init()
    ...
    local callBack = function (sender, eventType)
		if (eventType == ccui.TouchEventType.ended) then
            GetWayMgr:hideGetwayDropUI()
		end
	end
    bg1:addTouchEventListener(callBack)

    local titleTx       = bg2:getChildByName("title_tx")

    local titleName     = ''
    local dropId        = 0

    if self.obj['type'] == 'drop' then
        titleName       = self.obj['name']
        dropId          = self.obj['id']
    else
    ...
    for i = 1,15 do
        if dropConf['award' .. i] ~= '0' and #dropConf['award' .. i] > 0 then
            table.insert(randAwards,dropConf['award' .. i])
        end
    end
    ...
    local nameHeight = 40
    local itemHeight = 94 * scaleFactor
    local itemWidth = 94 * scaleFactor
    local colNum = 4
    local widhtOffset = 10
    local heightOffset = 10
    local leftInitPos = 15
    ...
    if fixAllNum > 0 then
        ...
        local title = cc.Label:createWithTTF(GlobalApi:getLocalStr_new("GETWAY_DES6"), "font/gamefont.ttf", 18)
        title:setAnchorPoint(cc.p(0.5,0))
        title:setPosition(cc.p(svSize.width/2,allHeight - height + 5))
        title:setColor(COLOR_TYPE.WHITE1)
        title:enableOutline(COLOROUTLINE_TYPE.PART_ATTR_GRAY, 2)
        ...
        for i = 1,fixAllNum do
            ...
            cell.awardBgImg:setScale(scaleFactor)
    ...
    if randAllNum > 0 then
        ...
        local title = cc.Label:createWithTTF(GlobalApi:getLocalStr_new("GETWAY_DES7"), "font/gamefont.ttf", 16)
        title:setAnchorPoint(cc.p(0.5,0))
        title:setPosition(cc.p(svSize.width/2,allHeight - height + 5))
        title:setColor(COLOR_TYPE.WHITE1)
        title:enableOutline(COLOROUTLINE_TYPE.PART_ATTR_GRAY, 2)
