...
function GetWayMgr:showGetwayUI(obj,showgetway,num,posobj,lv,ismerge)
	self.obj = obj
...
function GetWayMgr:getWayArr()
	...
	elseif typestr == 'equip' then
		conf = GameData:getConfData('equipconf')
	...
	elseif typestr == "group" then
		conf = GameData:getConfData('itemgroupbase')
	end
...
function GetWayMgr:getwayCountarr(ntype,index)
	...
	if ntype == 101 then
		...
		arrmaxcount = obj:getDayLimits(1)
		arrcount = obj:getDayLimits(1)-obj:getTimes(1)
		ispass = true
