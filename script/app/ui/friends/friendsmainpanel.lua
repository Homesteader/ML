...
function FriendsMainPanelUI:ctor()
    ...
    self.page = 1
    self.deleteState = false -- 是否处于删除状态
    self.deleteUids = {}    -- 等待删除的好友列表
end
...
function FriendsMainPanelUI:init()
    ...
    for i=1,3 do
        local arr = {}
        arr.btn = bgimg2:getChildByName('tab_'..i)
        arr.btntx = arr.btn:getChildByName('func_tx')
        ...
        self.friendbtnarr[i].btn:addTouchEventListener(function (sender, eventType)
            ...
            elseif eventType == ccui.TouchEventType.ended then
                ...
                if i ~= 1 then
                    FriendsMgr:showFriendsInfo(i)
                end    
            end
        end)

        if i == 1 then
            arr.btn:loadTextureNormal(defaultSel)
            arr.btntx:setColor(selectColor)
        else
            arr.btn:loadTextureNormal(defaultNor)
            arr.btntx:setColor(defaultColor)
        end

        if i == 3 then
            self.newfriendsimg = arr.btn:getChildByName('new_img')
        end
    end
    ...
    self.friendNumLabel = headpl:getChildByName('friend_num_label')
    self.friendNumLabel:setString(GlobalApi:getLocalStr_new('FRIENDS_DESC_8'))

    self.deleteTips = headpl:getChildByName('delete_tips')
    self.deleteTips:setString(GlobalApi:getLocalStr_new('FRIENDS_DESC_11'))

    -- 取消删除按钮
    self.cancelDeleteBtn = friendsbg:getChildByName('cancel_delete_btn')
    local cancelDeleteBtnTx = self.cancelDeleteBtn:getChildByName('func_tx')
    cancelDeleteBtnTx:setString(GlobalApi:getLocalStr_new('COMMON_STR_CANCLE'))
    self.cancelDeleteBtn:addTouchEventListener(function (sender, eventType)
        if eventType ==ccui.TouchEventType.began then
            AudioMgr.PlayAudio(11)
        elseif eventType == ccui.TouchEventType.ended then
    ...
    -- 提交删除按钮
    self.commitDeleteBtn = friendsbg:getChildByName('commit_delete_btn')
    local commitDeleteBtnTx = self.commitDeleteBtn:getChildByName('func_tx')
    commitDeleteBtnTx:setString(GlobalApi:getLocalStr_new('COMMON_STR_DELETE'))
    self.commitDeleteBtn:addTouchEventListener(function (sender, eventType)
        if eventType ==ccui.TouchEventType.began then
            AudioMgr.PlayAudio(11)
        elseif eventType == ccui.TouchEventType.ended then
            self:sendDeleteMsg()
    ...
    nofriendstx:setString(GlobalApi:getLocalStr_new('FRIENDS_DESC_1'))
    ...
    local deletebtn = headpl:getChildByName('delete_btn')
            ...
            if self.curFriendUid ~= nil then
                ...
                MessageMgr:sendPost('challenge', 'friend', json.encode(obj), function (jsonObj)
                    ...
                    elseif code == 103 then
                        promptmgr:showSystenHint(GlobalApi:getLocalStr_new('FRIENDS_MSG_DESC_2'), COLOR_TYPE.RED)
                    end
                end)
            end
        end
    ...
    -- bottompl
    self.giftbtn = bgimg2:getChildByName('havegift')
    self.giftbtn:addTouchEventListener(function (sender, eventType)
        if eventType == ccui.TouchEventType.began then
            AudioMgr.PlayAudio(11)
        elseif eventType == ccui.TouchEventType.ended then
            local judge = false
            ...
            if judge == false then
                promptmgr:showSystenHint(GlobalApi:getLocalStr_new('FRIENDS_MSG_DESC_3'), COLOR_TYPE.RED)
                return
            end
            ...
            MessageMgr:sendPost('deal_gift','friend',json.encode(obj),function (response)    
                ...
                if code == 0 then
                    ...
                    if (awards and #awards > 0) or isSendGift == true then
                        ...
                        promptmgr:showSystenHint(GlobalApi:getLocalStr_new('FRIENDS_MSG_DESC_4'), COLOR_TYPE.GREEN)
                    else
                        promptmgr:showSystenHint(GlobalApi:getLocalStr_new('FRIENDS_MSG_DESC_5'), COLOR_TYPE.GREEN)
                    end
                    ...
                    self.frienddata.dailyGiftGetNum = response.data.getNum or 0
                    self:update()
                end 
            end)
        end
    end)
    local btnpl = self.giftbtn:getChildByName('pl')
    local giftbtntx = btnpl:getChildByName('btntext')
    giftbtntx:setString(GlobalApi:getLocalStr_new('FRIENDS_DESC_3'))

    -- 领取次数显示
    self.getCountLabel = friendsbg:getChildByName('get_count_label')
    self.dailyGetCountTx = friendsbg:getChildByName('get_count_tx')
...
function FriendsMainPanelUI:sendDeleteMsg()
    local deleteNum = 0
    for k,v in pairs(self.deleteUids) do
    ...
    if deleteNum > 0 then
        ...
        MessageMgr:sendPost('remove', 'friend', json.encode(obj), function (response)    
            ...
            local data = response.data
            if code == 0 then
                promptmgr:showSystenHint(GlobalApi:getLocalStr_new('FRIENDS_MSG_DESC_1'), COLOR_TYPE.GREEN)

                for k,v in pairs(self.deleteUids) do
                    for k1,v1 in pairs(self.frienddata.friends) do
                        if tonumber(v1.uid) == tonumber(v) then
                            self.frienddata.friends[k1] = nil
                            break
                        end
                    end

                    self:refreshFriendData(v)

                    -- 清除聊天数据缓存
                    FriendsMgr:clearUserCacheData(v)
                end
        ...
        promptmgr:showSystenHint(GlobalApi:getLocalStr_new('FRIENDS_MSG_DESC_7'), COLOR_TYPE.RED)
    end
end
...
function FriendsMainPanelUI:update()
    ...
    if self.deleteState then
        self.cancelDeleteBtn:setVisible(true)
        self.commitDeleteBtn:setVisible(true)
        self.getCountLabel:setVisible(false)
        self.dailyGetCountTx:setVisible(false)
        self.giftbtn:setVisible(false)
        self.deleteTips:setVisible(true)
    else
        self.cancelDeleteBtn:setVisible(false)
        self.commitDeleteBtn:setVisible(false)
        self.getCountLabel:setVisible(true)
        self.dailyGetCountTx:setVisible(true)
        self.giftbtn:setVisible(true)
        self.deleteTips:setVisible(false)
    end
    ...
    self.dailyGetCountTx:setString(self.frienddata.dailyGiftGetNum .. '/' .. self.friendconf['dailyGiftGetLimit'].value)
end
...
function FriendsMainPanelUI:updateFriendListItem(index,bgimg)
    ...
    local exist = false
    local existIndex = 0
    for i,v in pairs(self.deleteUids) do
        if v == self.frienddataArr[index][2].uid then
            exist = true
            existIndex = i
            break
        end
    ...
    if self.deleteState then
        ...
        if exist then
            ...
            giftimg:loadTexture('uires/ui_new/common/finish.png')
        else
            giftimg:setVisible(false)
        end
        
        ShaderMgr:restoreWidgetDefaultShader(giftimg)
    else
        giftimg:setVisible(true)
        if self.frienddataArr[index][2].hasGiftReceive == 1 then
            ...
            if tonumber(self.frienddata.dailyGiftGetNum) >= tonumber(self.friendconf['dailyGiftGetLimit'].value) then
                -- 当日收礼物数已经超过上限
                giftimg:loadTexture('uires/ui_new/friend/gift.png')

                giftbtn:setTouchEnabled(false)
                ShaderMgr:setGrayForWidget(giftimg)
            else
                giftimg:loadTexture('uires/ui_new/friend/gift.png')
                ShaderMgr:restoreWidgetDefaultShader(giftimg)
            ...
            if self.frienddataArr[index][2].hasGiftSend == 0 then
                -- 还没有送过礼物
                giftimg:loadTexture('uires/ui_new/friend/gift_send.png')
                ShaderMgr:restoreWidgetDefaultShader(giftimg)
            else
                -- 已经送过了
                giftimg:loadTexture('uires/ui_new/friend/gift.png')
    ...
    giftbtn:addTouchEventListener(function (sender, eventType)
        ...
        elseif eventType == ccui.TouchEventType.ended then
            if self.deleteState then
                -- 设置/取消 删除标识
                if exist then
                    table.remove(self.deleteUids, existIndex)
                else
                    table.insert(self.deleteUids, self.frienddataArr[index][2].uid)
                end

                -- 刷新界面
                self:updateFriendListItem(index, self.friendscelltab[index])
            elseif self.frienddataArr[index][2].hasGiftReceive == 1 then   
    ...
    onlinetx:setString(GlobalApi:getLocalStr_new('FRIENDS_DESC_9'))
    ...
    if GlobalData:getServerTime() - active_time < lastActiveTime then
        onlinetx:setString(GlobalApi:getLocalStr_new("COMMON_STR_ONLINE"))
    ...
    if uid ~= nil then
        ...
        MessageMgr:sendPost('give_gift','friend',json.encode(obj),function (response)    
            ...
            if code == 0 then
                ...
                if name then
                    promptmgr:showSystenHint(string.format(GlobalApi:getLocalStr_new('FRIENDS_MSG_DESC_6'),name), COLOR_TYPE.GREEN)
                end
                self:updateFriendListItem(index,self.friendscelltab[index])
            end 
        end)
...
function FriendsMainPanelUI:initVoice()
    ...
    local function stopRecordLogic(maxRecordState)
        ...
        self.voiceBtn:loadTextureNormal('uires/ui/chat/anniu1.png')
    ...
    local function voiceBtnCallBack(sender, eventType)
        ...
        elseif eventType ==  ccui.TouchEventType.moved then
            ...
            --print('==============+++++++++++moved')
            self.voiceBtn:loadTextureNormal('uires/ui/chat/anniu2.png')
...
function FriendsMainPanelUI:createEditBox(attachNode)
    local maxLen = 64
    self.editbox = cc.EditBox:create(attachNode:getContentSize(), 'uires/ui/friends/input.png')
...
function FriendsMainPanelUI:switchVoiceState()
    ...
    if self.isVoice then    -- 语音状态
        self.swithBtn:loadTextureNormal("uires/ui_new/friend/keyboard.png")
    else
        self.swithBtn:loadTextureNormal("uires/ui_new/friend/siri.png")
    end

    self.swithBtn:setScale(0.74)
end
...
function FriendsMainPanelUI:SendChatMessage()
    ...
    if sendString == "" then
        promptmgr:showSystenHint(GlobalApi:getLocalStr_new('FRIENDS_DESC_39'), COLOR_TYPE.RED)
        return
    end
    if FriendsMgr:judgeSpeakIsOutTime() then
        promptmgr:showSystenHint(GlobalApi:getLocalStr('CHAT_DESC_1'), COLOR_TYPE.RED)
...
function FriendsMainPanelUI:updateRightMyCell(cell,index)
    ...
    if vip > 0 then
        ...
        vipLabel:setPosition(cc.p(315, imgV:getPositionY()))
