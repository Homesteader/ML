...
function ActivityMgr:getMenusAndCloseBtn2()
    if(self.uiClass.ActivityUI ~= nil) then
        local menus,closeBtn,recruitHelp = self.uiClass.ActivityUI:getMenusAndCloseBtn2()
        return menus,closeBtn,recruitHelp
    end
end
...
function ActivityMgr:getActivityTime(stype)
    ...
    elseif conf.type == 2 then -- 等级活动（七日狂欢）（七天乐）,许愿   
        ...
        if UserData:getUserObj().activity[stype] and UserData:getUserObj().activity[stype]['open_day'] then
    		time = UserData:getUserObj().activity[stype]['open_day']
    	end
