...
function LoginGoodGift:updateSV()
    ...
    for i = 1,num do
        ...
        tempCell:getChildByName('desc'):setString(string.format(GlobalApi:getLocalStr('LOGIN_GOOD_GIFT3'),i))
        
        local awardData = self.tempData[i].award
...
function LoginGoodGift:refreshDay(btn)
    ...
    --print('++++++++++++++++++++' .. allDays)
    local reward = UserData:getUserObj().activity.login_goodgift.reward -- ÖÁÉÙÎª{}
    if allDays >= day then
