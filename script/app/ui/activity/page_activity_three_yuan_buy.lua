...
function ActivityThreeYuanBuyUI:init()
    ...
    img:loadTexture('uires/ui/activity/three_yuan_img.png')
	...
	richText:setContentSize(cc.size(540, 40))

	local re1 = xx.RichTextLabel:create('仅需3元，即可在夏侯渊、赵云、甘宁、公孙瓒中任意合成一名武将，价值1200元宝，超低折扣终身仅限一次', 22, cc.c4b(254,227,134,255))
	...
	richText:setPosition(cc.p(310,277))
    ...
    for i = 1,4 do
        ...
        local heroconf = GlobalApi:getHeroConf(awards:getId())

	    local re1 = xx.RichTextLabel:create(heroconf.heroName, 24, awards:getNameColor())
        re1:setFont('font/gamefont.ttf')
        ...
        getBtn:addTouchEventListener(function (sender, eventType)
            ...
            elseif eventType == ccui.TouchEventType.ended then
                if self.data.money_buy2.status == 1 then -- 领取
                    ...
                    promptmgr:showMessageBox('只能领取一个武将的碎片80个，确认是否领取? ', MESSAGE_BOX_TYPE.MB_OK_CANCEL,function()
                        callBack()
                    end)
