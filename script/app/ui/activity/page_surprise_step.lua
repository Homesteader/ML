...
function SurpriseStep:initLeftBottom()
    ...
    local buyImg = self.rootBG:getChildByName('buy_img')
    local ownNowNum = 0
...
function SurpriseStep:stepLogic(step,costNum)
    ...
    if ownNowNum < costNum then
        promptmgr:showSystenHint(GlobalApi:getLocalStr('ACTIVE_SURPRIESE_STEP_DESC4'), COLOR_TYPE.RED)
        self:openBtn()
        return
    end
...
function SurpriseStep:disableBtn()
    local menus,closeBtn,recruitHelp = ActivityMgr:getMenusAndCloseBtn2()
    if menus then
