...
function GoldmineUI:init()
    ...
    self.ui["goldDepositBg"]:setPosition(cc.p(winsize.width-230, 56))
    ...
    infoNeiImg2:setPosition(cc.p(winsize.width/2-93, 48))
    ...
    self.ui["playernameLabel"] = cc.Label:createWithTTF("", "font/gamefont.ttf", 20)
    self.ui["playernameLabel"]:enableOutline(COLOROUTLINE_TYPE.BLACK, 1)
    self.ui["playernameLabel"]:enableShadow(GlobalApi:getLabelCustomShadow(ENABLESHADOW_TYPE.NORMAL))
    self.ui["playernameLabel"]:setAnchorPoint(cc.p(0.5, 1))
    self.ui["playernameLabel"]:setPosition(cc.p(65, 55))
    self.ui["playernameLabel"]:setMaxLineWidth(110)
    infoNeiImg:addChild(self.ui["playernameLabel"])
    ...
    for i = 1, NUM_MINE_TYPE do
        for j = 1, minesNum[i] do
            ...
            local labelBg = cc.Sprite:create("uires/ui/common/num_bg3.png")
...
function GoldmineUI:updateBottom()
    ...
    if cunrrMine then
        self.ui["playernameLabel"]:setString(cunrrMine.name)
        ...
        local _,heroCombatConf,heroModelConf = GlobalApi:getHeroConf(cunrrMine.main_role)
        self.ui["headpicBg"]:loadTexture(COLOR_FRAME[heroCombatConf.quality])
        ...
        self.currMineStatus = 1
        self.ui["playernameLabel"]:setString(GlobalApi:getLocalStr("STR_NOBODY_OCCUPY"))
