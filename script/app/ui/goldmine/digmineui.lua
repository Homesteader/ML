...
function DigMineUI:setUIBackInfo()
    UIManager.sidebar:setBackBtnCallback(UIManager:getUIName(GAME_UI.UI_DIGMINE), function (sender, eventType)
...
function DigMineUI:init()
    ...
    local upImg = self.bg_digmine:getChildByName('up_img')

    local wpos = cc.p(winsize.width / 2, winsize.height - 50)
    local lpos = self.bg_digmine:convertToNodeSpace(wpos)

    upImg:setPosition(lpos)
...
function DigMineUI:initEatDragon()
    local winsize           = cc.Director:getInstance():getWinSize()
    ...
    local title_tx          = bg_select:getChildByName("title_tx")

    local contentBg         = bg_select:getChildByName('content_bg')
    local infoBg            = contentBg:getChildByName('info_bg')

    local releaseBtn        = infoBg:getChildByName('release_btn')
    local eatBtn            = infoBg:getChildByName('eat_btn')

    local releaseBtnTx      = releaseBtn:getChildByName('tx')
    local eatBtnTx          = eatBtn:getChildByName('tx')

    title_tx:setString(GlobalApi:getLocalStr_new("SAVE_MR_DRAGON"))

    releaseBtnTx:setString(GlobalApi:getLocalStr_new("RELEASE_DIG_DRAGON"))
    eatBtnTx:setString(GlobalApi:getLocalStr_new("EAT_DIG_DRAGON"))

    local recoverNum        = GlobalApi:getGlobalValue("diggingDragonRecover")

    local rt1               = xx.RichText:create()

    rt1:setAnchorPoint(cc.p(0, 1))
    rt1:setPosition(cc.p(173, 112))
    rt1:setContentSize(cc.size(117, 20))

    local descStr = '<font color="#fef8d5ff" size="16" strokeColor="#6a3d1dff" strokeSize="1">' .. GlobalApi:getLocalStr_new("DIGMINE_EVENT_DESC_6") .. '</font>'
    descStr = string.format(descStr, '</font><font color="#7ff452ff" size="16" strokeColor="#463622ff" strokeSize="1">' .. recoverNum .. '</font><font color="#fef8d5ff" size="16" strokeColor="#6a3d1dff" strokeSize="1">')
   
    xx.Utils:Get():analyzeHTMLTag(rt1, descStr)

    bg_select:addChild(rt1)

    local rt2               = xx.RichText:create()

    rt2:setAnchorPoint(cc.p(0, 1))
    rt2:setPosition(cc.p(358, 84))
    rt2:setContentSize(cc.size(117, 20))

    local xxStr1            = GlobalApi:getLocalStr_new("DIGMINE_EVENT_DESC_7")
    local xxStr2            = GlobalApi:getLocalStr_new("DIGMINE_EVENT_DESC_8")
    local xxStr3            = GlobalApi:getLocalStr_new("DIGMINE_EVENT_DESC_9")

    local descStr = '<font color="#fef8d5ff" size="16" strokeColor="#6a3d1dff" strokeSize="1">' .. xxStr1 .. '</font>'
    descStr = string.format(descStr, '</font><font color="#7ff452ff" size="16" strokeColor="#463622ff" strokeSize="1">' .. xxStr2 .. '</font><font color="#fef8d5ff" size="16" strokeColor="#6a3d1dff" strokeSize="1">', '</font><font color="#7ff452ff" size="16" strokeColor="#463622ff" strokeSize="1">' .. xxStr3 .. '</font><font color="#fef8d5ff" size="16" strokeColor="#6a3d1dff" strokeSize="1">')
   
    xx.Utils:Get():analyzeHTMLTag(rt2, descStr)

    bg_select:addChild(rt2)
    ...
    releaseBtn:addTouchEventListener(function (sender, eventType)
        if eventType == ccui.TouchEventType.began then
            AudioMgr.PlayAudio(11)
        elseif eventType == ccui.TouchEventType.ended then
            local obj = {
    ...
    local dragonAni = GlobalApi:createLittleLossyAniByName("ui_jiulong")
    dragonAni:getAnimation():playWithIndex(0, -1, 1)
    dragonAni:setPosition(cc.p(276, 77.5))
    dragonAni:setScale(0.8)
    infoBg:addChild(dragonAni)

    local closeBtn = bg_select:getChildByName("close_btn")
...
--     local bg_progress = self.bg_digmine:getChildByName("bg_progress")
...
--     help_btn:setPosition(cc.p(1004, 670))
--     self.bg_digmine:addChild(help_btn)
-- end

function DigMineUI:initTopProgressBar()
    local upImg                 = self.bg_digmine:getChildByName('up_img')
    local bg_progress           = upImg:getChildByName("bg_progress")
    self.progress_bar           = bg_progress:getChildByName("progress_bar")
    ...
    for i = 1, 3 do
        local needNum           = diggingprogressConf[i].progress
        local toolNum           = diggingprogressConf[i].tool

        local needTx            = bg_progress:getChildByName('tx_' .. i)
        local box_node          = bg_progress:getChildByName("box_node_" .. i)
        ...
        needTx:setString(tostring(needNum))
        

        if tonumber(toolNum) > 0 then
            num_tx:setString('x' .. toolNum)
        else
            num_tx:setString('')
        end
        ...
        if i < 3 then
            box_node:setPositionX(barSize.width * needNum / self.maxTreasureNum)
        else
            box_node:setPositionX(barSize.width * needNum / self.maxTreasureNum - 40)
        end
        ...
        box_img:addTouchEventListener(function (sender, eventType)
            ...
            elseif eventType == ccui.TouchEventType.ended then
                if self.digBoxArr[i].status == 0 then
                    if i == 3 then
                        GetWayMgr:showGetwayUI(boxObj,false)
                    else
                        GetWayMgr:showGetwayUI2(boxObj, false)
                    end
                elseif self.digBoxArr[i].status == 1 then
...
function DigMineUI:initTopPl()
    local winsize       = cc.Director:getInstance():getWinSize()

    local upImg         = self.bg_digmine:getChildByName('up_img')

    local infoDesc1Bg   = upImg:getChildByName('info_desc_1_bg')

    local infoDesc1Tx   = infoDesc1Bg:getChildByName('tx_1')
    local infoDesc2Tx   = infoDesc1Bg:getChildByName('tx_2')

    local collectBg     = upImg:getChildByName('collect_bg')

    local titleTx       = collectBg:getChildByName('title_tx')

    local img1Light     = collectBg:getChildByName('img_1_light')

    local tx1           = collectBg:getChildByName('tx_1')
    local tx2           = collectBg:getChildByName('tx_2')
    local tx3           = collectBg:getChildByName('tx_3')

    img1Light:runAction(cc.RepeatForever:create(cc.RotateBy:create(4, 360)))

    infoDesc1Tx:setString( GlobalApi:getLocalStr_new('BAG_TAB_STR1') )
    infoDesc2Tx:setString( GlobalApi:getLocalStr_new('DROP_TO_USE_TEXT') )

    titleTx:setString( GlobalApi:getLocalStr_new('COLLECTED_TXT') )

    local collectNum    = self.maxTreasureNum - self.treasureNum

    tx1:setString('x' .. collectNum)
    -- tx2:setString(self.enemyNum)
    -- tx3:setString(self.boxNum)
    ...
    for i = 1, 2 do
        if diggingbombConf[i] then
            ...
            upImg:addChild(self.bombItemImg[i])
            ...
            local confirmBombRedImg = ccui.ImageView:create("uires/ui_new/common/bg_red2.png")
            confirmBombRedImg:setScale9Enabled(true)
            ...
            local confirmImg    = ccui.ImageView:create("uires/ui_new/digmine/confirm_img_new.png")
            local cancelImg     = ccui.ImageView:create("uires/ui_new/digmine/cancel_img_new.png")
            ...
            local awardCell = ClassItemCell:create(ITEM_CELL_TYPE.ITEM, self.bombObjs[i], upImg)
            awardCell.awardBgImg:setPosition(cc.p(300 + (i - 1) * 90, 61))
            awardCell.awardBgImg:setScale(0.7)
            ...
            awardCell.awardBgImg:addTouchEventListener(function (sender, eventType)
                if eventType == ccui.TouchEventType.began then
                    ...
                    startPos    = sender:getTouchBeganPosition()
                    itemCurrPos = upImg:convertToWorldSpace(cc.p(awardCell.awardBgImg:getPosition()))
                elseif eventType == ccui.TouchEventType.canceled then
                    ...
                    if movePos then
                        ...
                        if mineObj and mineObj.mineType == MINE_TYPE.EMPTY then
                            ...
                            promptmgr:showSystenHint(GlobalApi:getLocalStr_new("DIG_MINE_BOMB_TIPS_1"), COLOR_TYPE.RED)
                        end
                    end
                    self.bombItemImg[i]:setVisible(false)
                    startShow = false
                ...
                elseif eventType == ccui.TouchEventType.moved then
                    ...
                    self.bombItemImg[i]:setPosition(upImg:convertToNodeSpace(movePos))
                end
            end)
        end
    end

    local bg_meat           = upImg:getChildByName("bg_meat")

    bg_meat:setVisible(false)
...
function DigMineUI:updateLeftPl()
    ...
    UserData:getUserObj():setDiggingLeftTimes( self.digTimes )
    ...
    -- print('maxnum = ' .. self.maxTreasureNum)
    -- print('treasurenum = ' .. self.treasureNum)
...
function DigMineUI:initMines()
    ...
    local rowWidth = MAX_MINES_NUM_ROW*MINE_WIDTH + MINE_SPACE_ROW*(MAX_MINES_NUM_ROW + 1)
    local colHeight = MAX_MINES_NUM_COL*MINE_HEIGHT + MINE_SPACE_COL*(MAX_MINES_NUM_COL + 1)
    local bg_mine = ccui.ImageView:create("uires/ui_new/digmine/bg_mine.png")
    ...
    for i = 1, bgMineRowNum do
        for j = 1, bgMineColNum do
            if i ~= 1 or j ~= 1 then
                local bg_mine2 = ccui.ImageView:create("uires/ui_new/digmine/bg_mine.png")
...
function DigMineUI:createNormalMine(rowIndex, colIndex)
    local normalMineImg = ccui.ImageView:create("uires/ui_new/digmine/mine_1.png")
        ...
        if not self.moveFlag then
            if self.minesObj[rowIndex][colIndex] and self.minesObj[rowIndex][colIndex].clickable then
                if self.digTimes > 0 then
                    ...
                    local cash = UserData:getUserObj():getCash()
                    if cash < costCash then -- 元宝不足
                        promptmgr:showMessageBox(GlobalApi:getLocalStr_new("NOT_ENOUGH_GOTO_BUY"), MESSAGE_BOX_TYPE.MB_OK_CANCEL,function()
                            GlobalApi:getGotoByModule("cash")
                        end,GlobalApi:getLocalStr_new("MESSAGE_GO_CASH"),GlobalApi:getLocalStr_new("MESSAGE_NO"))
                    else
                        promptmgr:showMessageBox(string.format(GlobalApi:getLocalStr_new("MINE_BUY_TOOL"), costCash, tonumber(GlobalApi:getGlobalValue("diggingToolPerDeal"))), MESSAGE_BOX_TYPE.MB_OK_CANCEL, function ()
                            MessageMgr:sendPost("buy_tool", "digging", "{}", function(jsonObj)
                                if jsonObj.code == 0 then
                                    self.buyTimes = self.buyTimes + 1
                                    self.digTimes = jsonObj.data.tool or self.digTimes
                                    ...
                                    self:updateLeftPl()
                                    promptmgr:showSystenHint(GlobalApi:getLocalStr_new("SUCCESS_BUY"), COLOR_TYPE.GREEN)
                                end
                            end)
                        end)
                    end
                ...
                promptmgr:showSystenHint(GlobalApi:getLocalStr_new("MINE_NO_DIG"), COLOR_TYPE.RED)
            end
        end
    end)
...
function DigMineUI:createDoor(rowIndex, colIndex)
    local doorImg = ccui.ImageView:create("uires/ui_new/digmine/door.png")
...
function DigMineUI:createAdamas(rowIndex, colIndex)
    local adamasImg = ccui.ImageView:create("uires/ui_new/digmine/mine_2.png")
    adamasImg:setTouchEnabled(true)
    adamasImg:addClickEventListener(function ()
        if not self.moveFlag then
            promptmgr:showSystenHint(GlobalApi:getLocalStr_new("CAN_NOT_DIG_ADAMAS"), COLOR_TYPE.RED)
        end
    end)
...
function DigMineUI:createTreasure(rowIndex, colIndex)
    ...
    local treasureBgImg = ccui.ImageView:create("uires/ui_new/digmine/mine_1.png")
    local treasureLight = ccui.ImageView:create("uires/ui_new/digmine/digmine_unknow_light.png")
    treasureLight:setPosition(cc.p(MINE_WIDTH/2, MINE_HEIGHT/2))
    treasureBgImg:addChild(treasureLight)
    local treasureImg = ccui.ImageView:create("uires/ui_new/digmine/question_ico.png")
...
function DigMineUI:createBox(mineType, rowIndex, colIndex, boxType)
    ...
    local boxImg = ccui.ImageView:create("uires/ui_new/digmine/digmine_box.png")
    ...
    boxImg:addClickEventListener(function ()
        if not self.moveFlag then
            ...
            if UserData:getUserObj():getCash() >= needCash then
                ...
                promptmgr:showMessageBox(string.format(GlobalApi:getLocalStr_new("CASH_NOT_ENOUGH_GOTO_RECHARGE"), needCash), MESSAGE_BOX_TYPE.MB_OK_CANCEL,function()
                        GlobalApi:getGotoByModule("cash")
                    end,GlobalApi:getLocalStr_new("MESSAGE_GO_CASH"),GlobalApi:getLocalStr_new("MESSAGE_NO"))
            end
        end
...
function DigMineUI:createEnemy(rowIndex, colIndex, mineTypeName, enemyLevel)
    ...
    if enemyLevel > 1 then
        enemyImg = ccui.ImageView:create("uires/ui_new/digmine/digmine_monster1.png")
    else
        enemyImg = ccui.ImageView:create("uires/ui_new/digmine/digmine_monster2.png")
    end
...
function DigMineUI:showLightEffect(rowIndex, colIndex, mineObj)
    local lightImg = ccui.ImageView:create("uires/ui_new/digmine/mine_light.png")
...
function DigMineUI:showEatMeat(addDigTimes, callback)
    ...
    if self.eatMeatImg then
        ...
        self.eatMeatImg = ccui.ImageView:create("uires/ui_new/common/bg1_gray1.png")
    ...
    self.addDigTimesTx:setString(GlobalApi:getLocalStr_new("STR_VITALITY") .. "+" .. addDigTimes)
...
function DigMineUI:showGuideImg()
    local bgImg = ccui.ImageView:create('uires/ui_new/common/bg1_gray11.png')
    ...
    local guideImg = ccui.ImageView:create('uires/ui_new/digmine/guide_digmine.png')
...
function DigMineUI:openRewardBox(index)
    ...
    MessageMgr:sendPost("click_box", "digging", json.encode(args), function(jsonObj)
        if jsonObj.code == 0 then
            ...
            ShaderMgr:setGrayForWidget(self.digBoxArr[index].box)
...
function DigMineUI:doBuyMeat()
    if self.digTimes >= self.maxTims then
        promptmgr:showSystenHint(GlobalApi:getLocalStr_new("TIMES_FULL_NO_NEED_BUY"), COLOR_TYPE.RED)
    else
        local buyConf   = GameData:getConfData("buy")
        local costCash  = nil
        if self.buyTimes >= #buyConf then
