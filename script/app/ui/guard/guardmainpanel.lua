...
function GuardMainUI:updateLeft(status)
	...
	patrolLeftPl:setBackGroundImage('uires/ui/guard/'..cityConf.bgimg)
	myHeroLeftPl:setBackGroundImage('uires/ui/guard/'..cityConf.bgimg)
	addLeftPl:setBackGroundImage('uires/ui/guard/'..cityConf.bgimg)
	enemyLeftPl:setBackGroundImage('uires/ui/guard/'..cityConf.bgimg)
	...
	if status == 1 then
		...
		local bossConf,bossCombatConf,bossModelConf = GlobalApi:getMonsterConf(tonumber(bossId))
		
		local spineAni = GlobalApi:createLittleLossyAniByName(bossModelConf.modelUrl.."_display")
		local talkbg = enemyLeftPl:getChildByName('talk_bg')
		local talktx = talkbg:getChildByName('talk_tx')
		talktx:setString(monsterConf.content)
	    spineAni:setScale(1)
	    spineAni:setPosition(cc.p(185,100))
	    spineAni:getAnimation():play('idle', -1, 1)
	    enemyLeftPl:addChild(spineAni,1,1)
	    talkbg:setLocalZOrder(2)
	...
	elseif status == 4 then
		...
		for k, v in pairs(data.guard.field) do
			if tonumber(k) == self.selectId then
			    ...
			    if heroModelConf.uiOffsetY ~= 0 then
			    	spine:setPosition(cc.p(25, 200-heroModelConf.uiOffsetY+30))
			    end
			    spine:setAnimation(0, "animation", true)
			    self.selectRole:addChild(spine,1,1000)

			    local move1 = cc.MoveTo:create(3, cc.p(200, 150))
			    local move2 = cc.MoveTo:create(3, cc.p(500, 100))
			    local move3 = cc.MoveTo:create(3, cc.p(100, 150))
			    local move4 = cc.MoveTo:create(3, cc.p(-100, 100))
...
function GuardMainUI:addIdleNpc(parent)
    ...
    talktx1:setFontName('font/gamefont1.TTF')
