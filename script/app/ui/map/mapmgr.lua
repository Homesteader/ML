local ClassLevelMapUI = require("script/app/ui/map/mapui_new")
...
function MapMgr:showAwardPanel()
	if self.uiClass['ShowAwardUI'] == nil then
		...
		-- printall(awards)
		if #awards > 0 then
			self.uiClass['ShowAwardUI'] = ClassShowAwardUI.new(awards,MapData:getFightedCityId(),1)
			self.uiClass['ShowAwardUI']:showUI()
		elseif #equipTab > 0 then
			if fightedId == 10 then
				self.uiClass['ShowAwardUI'] = ClassShowAwardUI.new(equipTab,MapData:getFightedCityId(),2)
				self.uiClass['ShowAwardUI']:showUI()
			else
...
function MapMgr:showExpeditionPanel(customsId)

	if self.uiClass['ExpeditionPanelUI'] == nil then
		self.uiClass['ExpeditionPanelUI'] = ClassExpeditionUI.new(customsId)
...
function MapMgr:showMainScene(ntype,customsId,moveCustomsId,isCity,callback)
	if self.uiClass['LevelMapUI'] == nil then
		UIManager:closeAllUI()
		local curCityId = MapData:getCurProgress()
		customsId = customsId or curCityId
		self.uiClass['LevelMapUI'] = ClassLevelMapUI.new(ntype,customsId,moveCustomsId,isCity)
		self.uiClass['LevelMapUI']:showUI()
		if callback then
			callback()
		end
	end
...
function MapMgr:hideMainScene()
	if self.uiClass['LevelMapUI'] then
		self.uiClass['LevelMapUI']:hideUI()
		self.uiClass['LevelMapUI'] = nil
	end
end

function MapMgr:closeMapOnshow()
	if self.uiClass['LevelMapUI'] then
		self.uiClass['LevelMapUI']:closeMapOnshow()
	end
end
...
-- battleType 战斗类型
-- cityId 城池ID
-- customsType 1 普通 2精英 3噩梦
function MapMgr:playBattle(battleType,customsId,customsType,callack)
	...
	BattleMgr:playBattle(battleType, obj, callack)
end
		...
		if self.uiClass['PrefecturePanelUI'] == nil then
			...
			MessageMgr:sendPost('lord_get','battle',json.encode(args),function (response)
				...
				if code == 0 then
	                ...
	                MapData:setLordDrop(data.self_lord_drop)
					self.uiClass['PrefecturePanelUI'] = ClassPrefectureUI.new(id,data.owner)
