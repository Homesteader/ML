...
function PrefectureUI:ctor(id,owner)
...
function PrefectureUI:timeoutCallback(img,img1,tx,pos)
    ...
    if self.owner.time then
        local diffTime = self.owner.time - GlobalData:getServerTime() + GlobalApi:getGlobalValue('lordMaxHour')*3600
...
function PrefectureUI:updatePanel()
    ...
    if self.owner.hid then
        ...
        self.root:runAction(cc.Sequence:create(cc.DelayTime:create(0.1),cc.CallFunc:create(function ()
			...
			local heroBaseConf,heroCombatConf = GlobalApi:getHeroConf(tonumber(self.owner.hid))
            ...
            if spineAni then
                ...
                spineAni:setPosition(cc.p(170,200))
    ...
    if MapData.lordId == self.id then
        ...
        if not self.foodRt then
            ...
            richText:setPosition(cc.p(740,65))
