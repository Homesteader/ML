...
local ClassRankingListNewUI = require("script/app/ui/rankinglist/rankinglistmain_new")
...
function RankingListMgr:showRankingListMain(page, subpage,menutab)
	...
	if self.uiClass.rankingListV3UI == nil then
		...
		self.uiClass.rankingListV3UI = ClassRankingListNewUI.new(menutab)
	end
	...
	-- 	return
	-- end
	self.uiClass.rankingListV3UI:showUI(UI_SHOW_TYPE.MOVEINL)
...
function RankingListMgr:hideRankingListMain()
	if self.uiClass.rankingListV3UI then
		self.uiClass.rankingListV3UI:hideUI(UI_HIDE_TYPE.MOVEOUTR)
