    ...
    for i=1,#self.data do
        ...
        if self.data[i].attacker == 1 then
            if self.data[i].win == true then
                resultImg:loadTexture('uires/ui/worldwar/yellow_a.png')
            else
                resultImg:loadTexture('uires/ui/worldwar/blue_a.png')
            end
        else
            if self.data[i].win == true then
                resultImg:loadTexture('uires/ui/worldwar/yellow_d.png')
            else
                resultImg:loadTexture('uires/ui/worldwar/blue_d.png')
            end
        end
        ...
        revengeBtn:addTouchEventListener(function (sender, eventType)
            ...
            elseif eventType == ccui.TouchEventType.ended then
                ...
                if tonumber(WorldWarMgr.battleTimes) >= WorldWarMgr.maxBattleTimes then
                    promptmgr:showMessageBox(string.format(getLocalStringValue('E_STR_ARENA_NO_FREE_COUNT'),WorldWarMgr.maxBuyBattle),MESSAGE_BOX_TYPE.MB_OK_CANCEL,function()
