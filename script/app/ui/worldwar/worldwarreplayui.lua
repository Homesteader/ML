...
function WorldWarReplayUI:createSpine()
    ...
    if not self.spineAniA then
		...
		local heroBaseConf,heroCombatConf = GlobalApi:getHeroConf(tonumber(playerA.max_hid))
    ...
    if not self.spineAniB then
		...
		local heroBaseConf,heroCombatConf = GlobalApi:getHeroConf(tonumber(playerB.max_hid))
        if heroCombatConf.camp == 5 then
    ...
    for i=1,5 do
        if self.replays[i] then
            ...
            if self.replays[i].win + 1 == playerAid then
                self.cells[i].cellImg:loadTexture('uires/ui/worldwar/red_bg.png')
                self.cells[i].victoryImg:loadTexture('uires/ui/worldwar/red_victory.png')
                self.cells[i].nameTx:setString(playerA.name)
                self.cells[i].serverTx:setString(playerA.sid..' '..GlobalApi:getLocalStr('FU'))
                numA = numA + 1
            else
                self.cells[i].cellImg:loadTexture('uires/ui/worldwar/blue_bg.png')
                self.cells[i].victoryImg:loadTexture('uires/ui/worldwar/blue_victory.png')
