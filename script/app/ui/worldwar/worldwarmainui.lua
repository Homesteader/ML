...
function WorldWarMainUI:updatePanel()
    ...
    if not self.rts then
        ...
        richText:setPosition(cc.p(286,22))
    ...
    for i=1,3 do
        ...
        if WorldWarMgr.top3[i] then
            ...
            if not self.roleSpines[i] then
				...
				local heroBaseConf,heroCombatConf = GlobalApi:getHeroConf(tonumber(WorldWarMgr.top3[i].max_hid))
...
function WorldWarMainUI:init()
    ...
    leftTopImg:setPosition(cc.p(0,winSize.height))
