...
function ChatUI:createEditBox(attachNode)
    local maxLen = 64
    self.editbox = cc.EditBox:create(cc.size(attachNode:getContentSize().width - 10,attachNode:getContentSize().height), 'uires/ui/chat/bg_inputzone.png')
...
function ChatUI:changeToChannel(channelString)
    ...
    for i = 1,self.numOChannels do
        if(self.channelStrings[i] == channelString) then
            self.channelBtns[i]:loadTextureNormal('uires/ui/chat/btn_light.png')
            self.channelTexts[i]:setTextColor(COLOR_TYPE.PALE)
        else
            self.channelBtns[i]:loadTextureNormal('uires/ui/chat/btn_dark.png')
...
function ChatUI:switchVoiceState()
    if (self.currentChannel == "world") or (self.currentChannel == "legion") then
        ...
        if self.isVoice then    -- 语音状态
            self.swithBtn:loadTexture("uires/ui/chat/jianpan.png")
        else
            self.swithBtn:loadTexture("uires/ui/chat/huatong.png")
        end
