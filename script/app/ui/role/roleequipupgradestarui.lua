...
local RoleEquipUpgradeStarUI = class("RoleEquipUpgradeStarUI", ClassRoleBaseUI)
...
function RoleEquipUpgradeStarUI:ctor()
...
function RoleEquipUpgradeStarUI:initPanel()
    self.panel = cc.CSLoader:createNode("csb/roleequipupgradestarpanel.csb")
    local equipBgImg = self.panel:getChildByName("equip_bg_img")
    local equipImg = equipBgImg:getChildByName("equip_img")
    local equipIconNode = equipImg:getChildByName("equip_icon_node")
    self.equipIconNode = equipIconNode
    self.equipTab = ClassItemCell:create(ITEM_CELL_TYPE.ITEM)
    self.equipTab.awardImg:ignoreContentAdaptWithSize(true)
    equipIconNode:addChild(self.equipTab.awardBgImg)
    self.equipInfoLabel = equipImg:getChildByName("info_tx")

    self.equipArr = {}
    for i = 1, 6 do
        local equipIconNode = equipImg:getChildByName("equip_icon_node_" .. i)
        self.equipArr[i] = {}
        local tab = ClassItemCell:create(ITEM_CELL_TYPE.ITEM)
        tab.awardImg:ignoreContentAdaptWithSize(true)
        self.equipArr[i].equipTab = tab
        ...
        tab.awardBgImg:addTouchEventListener(function (sender, eventType)
            ...
            if eventType == ccui.TouchEventType.ended then
                if self.equipObj:getGodLevel() >= 10 then
                    -- 已经满级了
                ...
                local equipSelectUI = ClassEquipSelectUI.new(roleobj,self.selectedMap, 6, 0, 4, 0, function (selectedMap)
                    ...
                    self.selectedArr = {}
                    for i=1,6 do
                        ClassItemCell:setGodLight(self.equipArr[i].equipTab.awardBgImg, 0)
                        self.equipArr[i].equipTab.starImg:setVisible(false)
                    end
                    for k, v in pairs(selectedMap) do
                        self.selectedMap[k] = v
                        table.insert(self.selectedArr, v)
                    end
                    RoleMgr:setDirty("RoleMainUI")
                end)
    ...
    self.godStar = {}
    for i = 1, 10 do
        local starBg = equipImg:getChildByName("star_bg_" .. i)
        self.godStar[i] = starBg:getChildByName("star_img")
    end

    local progressBg = equipImg:getChildByName("progress_bg")
    ...
    -- 自动筛选
    local selectBtn = equipBgImg:getChildByName("select_btn")
    ...
    self.swallowBtn = equipBgImg:getChildByName("swallow_btn")
        ...
        if eventType == ccui.TouchEventType.ended then
            ...
            local function sendMsg()
                ...
                MessageMgr:sendPost("swallow", "equip", json.encode(args), function (jsonObj)
                    ...
                    if code == 0 then
                        -- action!
                        local size = self.equipTab.awardBgImg:getContentSize()
                        local pos = self.equipTab.awardBgImg:convertToWorldSpace(cc.p(size.width/2, size.height/2))
                        ...
                        for i,v in ipairs(self.selectedArr) do
                            -- print(i,v)
                            local oriX, oriY = self.equipArr[i].equipTab.awardBgImg:getPosition()
                            local fSprite = self.equipArr[i].equipTab.awardBgImg:clone()
                            local eSprite = self.equipArr[i].equipTab.awardImg:clone()
                            local despos = self.equipArr[i].equipIconNode:convertToNodeSpace(cc.p(pos.x,pos.y))

                            fSprite:addChild(eSprite)
                            self.equipArr[i].equipTab.awardBgImg:loadTexture(COLOR_ITEMFRAME.DEFAULT)
                            self.equipArr[i].equipTab.awardImg:loadTexture(DEFAULTEQUIP[3])
                            self.equipArr[i].equipTab.awardBgImg:getParent():addChild(fSprite)
...
function RoleEquipUpgradeStarUI:update(roleObj, equipPos)
...
function RoleEquipUpgradeStarUI:updateUI()
    local equipObj = self.equipObj
    local godLevel = equipObj:getGodLevel()
    ClassItemCell:updateItem(self.equipTab, equipObj, 1)
    ClassItemCell:setGodLight(self.equipTab.awardBgImg, equipObj:getGodId())
    ...
    for k, v in ipairs(self.selectedArr) do
        ClassItemCell:updateItem(self.equipArr[self.selectIndex].equipTab, v, 1)
        ClassItemCell:setGodLight(self.equipArr[self.selectIndex].equipTab.awardBgImg, v:getGodId())
    ...
    if equipObj:getGodLevel() >= 10 then
        -- 已经满级了
...
function RoleEquipUpgradeStarUI:onMoveOut()
    self.selectedMap = {}
    self.selectedArr = {}
    for i=1,6 do
        ClassItemCell:setGodLight(self.equipArr[i].equipTab.awardBgImg,0)
...
function RoleEquipUpgradeStarUI:disableButtonClick()
    self.swallowBtn:setTouchEnabled(false)
    self.selectBtn:setTouchEnabled(false)
    for k, v in ipairs(self.equipArr) do
        v.equipTab.awardBgImg:setTouchEnabled(false)
    end
end

function RoleEquipUpgradeStarUI:enableButtonClick()
    self.swallowBtn:setTouchEnabled(true)
    self.selectBtn:setTouchEnabled(true)
    for k, v in ipairs(self.equipArr) do
        v.equipTab.awardBgImg:setTouchEnabled(true)
    end
end

return RoleEquipUpgradeStarUI