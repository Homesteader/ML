...
local function getSoldierAttribute(role)
	...
	if conf ~= nil then
		for i = 2, MAXSOLDIERLV do
			...
			if tonumber(conf[i - 1].num) ~= tonumber(conf[i].num) then
...
function SoldierUpgradeUI:init()
	...
	local star = self.role:getSoldierStar()
	self:_tipSoldier(level,star)
			...
			tip_frame:runAction(cc.Sequence:create(
				...
				cc.CallFunc:create(function ()
					...
					bg:getChildByName('press_tx')
						:setVisible(true)
						:setString(GlobalApi:getLocalStr('CLICK_SCREEN_CONTINUE'))
						:setFontName('font/gamefont1.TTF')
						:runAction(cc.RepeatForever:create(
...
function SoldierUpgradeUI:_tipSoldier(level,star)
	...
	local soldierType = self.role:getSoldierId()
	local soldlevelconf = GameData:getConfData('soldierlevel')[soldierType][level][star]
	page:getChildByName('soldier_img'):loadTexture('uires/ui/role/' ..soldlevelconf.soldierIcon)

	local soldierid = soldlevelconf.soldierId
	local soldierName = GameData:getConfData('soldier')[soldierid].soldierName
	page:getChildByName('title_tx'):setString(soldierName)
...
function SoldierUpgradeUI:step2_number(a)
	...
	for i = 1, 12 do
		local img = ccui.ImageView:create('uires/ui/role/' .. conf.soldierIcon)
        ...
        -- 底座
        local imgDizuo = ccui.ImageView:create('uires/ui/role/dizuo.png')
...
function SoldierUpgradeUI:step2_skill(a)
	...
	local light = ccui.ImageView:create('uires/ui/strength/light_bg.png')
...
function SoldierUpgradeUI:step2_newtype(a)
	...
	local nameTx = cc.Label:createWithTTF("", "font/gamefont1.TTF", 30)
    nameTx:setPosition(cc.p(size.width/2, size.height/2))
    nameTx:setColor(COLOR_QUALITY[quality + 1])
    nameTx:enableOutline(COLOROUTLINE_QUALITY[quality + 1], 1)
	...
	local oriimg = ccui.ImageView:create('uires/ui/role/' .. conf[level].soldierIcon)
    ...
    flashsp:runAction(cc.Sequence:create(
    	...
    	cc.CallFunc:create(function()
			...
			local lightImg = ccui.ImageView:create('uires/ui/common/light.png')
			...
			local nameTx1 = cc.Label:createWithTTF("", "font/gamefont1.TTF", 30)
		    nameTx1:setPosition(cc.p(size.width/2, size.height/2))
		    nameTx1:setColor(COLOR_QUALITY[quality1 + 1])
		    nameTx1:enableOutline(COLOROUTLINE_QUALITY[quality + 1], 1)
			...
			local tarimg = ccui.ImageView:create('uires/ui/role/' .. conf[level + 1].soldierIcon)
