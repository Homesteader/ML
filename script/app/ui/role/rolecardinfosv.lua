...
local RoleCardInfoSv = class("RoleCardInfoSv", ClassRoleBaseUI)
...
function RoleCardInfoSv:initPanel()
	self.panel = cc.CSLoader:createNode("csb/rolecardinfosv.csb")
	local bgimg = self.panel:getChildByName('bg_img1')
...
function RoleCardInfoSv:genTitle(idx)
	local titletx = GlobalApi:getLocalStr(titleImg[idx])
	local title = cc.Label:createWithTTF(titletx, "font/gamefont1.TTF", 25)
...
function RoleCardInfoSv:genBaseAttr()
...
function RoleCardInfoSv:genFightAttr()
	local ret = cc.Node:create()
	...
	skillRichText:setPosition(cc.p(leftInterval, verticalInterval))
 	for i=1,4 do
 		...
 		local re2 = xx.RichTextLabel:create(str, 20, cc.c4b(36, 255, 0, 255))
...
function RoleCardInfoSv:genSkill()
	local ret = cc.Node:create()
...
-- fate stay night ... 
function RoleCardInfoSv:genFateStayNight()
	local ret = cc.Node:create()
	...
	if #fateatt > 1 then
		...
		for i = #fateatt, 1, -1 do
			local panel = fateUI:genFateStayNight(self.obj, i, nil)
...
function RoleCardInfoSv:genTalent()
	local ret = cc.Node:create()
...
function RoleCardInfoSv:update(obj)
	self.obj = obj
...
function RoleCardInfoSv:onMoveOut()

end

function RoleCardInfoSv:updateHeight()
	local sh = 0
