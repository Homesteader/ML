...
function StrengthenPopupUI:playUpgradeStarAction()
	...
	flagImg:loadTexture('uires/ui/role/flag_'..conf.quality..'.png')
	if conf.quality ~= conf1.quality then
		flagOldImg:loadTexture('uires/ui/role/flag_'..conf1.quality..'.png')
		flagImg:setScale(10)
			...
			if self.type == 'upgrade_junzhu' then
				self.spine_frame:runAction(cc.Sequence:create(
					...
					cc.CallFunc:create(function (  )
						...
						local mask = ccui.ImageView:create('uires/ui/tavern/mask_white.png')
		...
		self.spine_frame:runAction(cc.Sequence:create(
			...
			cc.CallFunc:create(function()
						...
						if self.type == 'upgrade_junzhu' then
							...
							cc.CallFunc:create(function()
									-- local sp = cc.Sprite:create('uires/ui/strength/fazhen1.png')
						...
						local _,_,heroModelConf = GlobalApi:getHeroConf(self.role:getId())
...
function StrengthenPopupUI:getDescRichText()
	if self.targetlv > 1 then
		...
		local re2 = xx.RichTextLabel:create('   '..GlobalApi:getLocalStr("AREADY_ACTIVE"),24,COLOR_TYPE.YELLOW)
	    ...
	    richText:setPosition(cc.p(350,15))
	    return richText
	else
	    ...
	    local re7 = xx.RichTextLabel:create('   '..GlobalApi:getLocalStr("AREADY_ACTIVE"),24,COLOR_TYPE.YELLOW)
	    re7:setFont('font/gamefont1.TTF')
	    re7:setStroke(COLOROUTLINE_TYPE.YELLOW, 1)
	    richText:addElement(re7)
	    richText:setAnchorPoint(cc.p(0,1))
	    richText:setPosition(cc.p(315,15))
	    return richText
	end
...
function StrengthenPopupUI:initAttrFrame()
	...
	for i,v in ipairs(self.idx) do
		...
		elseif self.type == 'upgrade_risestar' then
			...
			self.flagImgs[v]:loadTexture('uires/ui/role/flag_'..conf.quality..'.png')
