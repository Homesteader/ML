...
local ClassRoleEquipRebuildUI = require('script/app/ui/role/roleequiprebuild')
local ClassRoleEquipOneKeyRebuildUI = require('script/app/ui/role/roleequiponekeyrebuild')
cc.exports.RoleMgr = {
	uiClass = {
		RoleMainUI = nil,
		RoleListUI = nil,
		...
		RoleEquipRebuildUI = nil,
		RoleEquipOneKeyRebuildUI = nil,
	},
	openType = nil,
	rolelistdirty = false,
--	dirty = false,
...
function RoleMgr:showRoleSelectListOutSide(obj,type)
	if self.uiClass["RoleSelectListOutSideUI"] == nil then
		self.uiClass["RoleSelectListOutSideUI"] = ClassRoleSelectListOutSideUI.new(obj,type)
...
function RoleMgr:showEquipRebuildUI(ntype,obj,selectpos)

	if not self.uiClass["RoleEquipRebuildUI"]  then

		self.uiClass["RoleEquipRebuildUI"] = ClassRoleEquipRebuildUI.new(ntype,obj,selectpos)
		self.uiClass["RoleEquipRebuildUI"]:showUI()
	end
end

function RoleMgr:updateEquipRebuildUI( )
	if self.uiClass["RoleEquipRebuildUI"]  then
		self.uiClass["RoleEquipRebuildUI"]:update()
	end
end

function RoleMgr:hideEquipRebuildUI( )
	if self.uiClass["RoleEquipRebuildUI"]  then
		self.uiClass["RoleEquipRebuildUI"]:hideUI()
		self.uiClass["RoleEquipRebuildUI"] = nil
	end
end

function RoleMgr:showEquipOnekeyRebuildUI(buildType,obj)

	if not self.uiClass["RoleEquipOneKeyRebuildUI"]  then

		local modulekey = ntype == 1 and 'oneKeyEquipUpgrade' or 'oneKeyEquipRefine'
		local desc,notOpenCode = GlobalApi:getGotoByModule_new(modulekey,true)
		if desc then 
			promptmgr:showSystenHint(desc, COLOR_TYPE.RED)
		else
			self.uiClass["RoleEquipOneKeyRebuildUI"] = ClassRoleEquipOneKeyRebuildUI.new(buildType,obj)
			self.uiClass["RoleEquipOneKeyRebuildUI"]:showUI()
		end
	end
end

function RoleMgr:hideEquipOnekeyRebuildUI( )
	if self.uiClass["RoleEquipOneKeyRebuildUI"]  then
		self.uiClass["RoleEquipOneKeyRebuildUI"]:hideUI()
		self.uiClass["RoleEquipOneKeyRebuildUI"] = nil
	end
end
...
function RoleMgr:refreshPartPanel( )
	if self.uiClass["RoleMainUI"] ~= nil then
		self.uiClass["RoleMainUI"]:refreshPartPanel()
	end
end

function RoleMgr:refreshPartAwakePanel( )
	if self.uiClass["RoleMainUI"] ~= nil then
		self.uiClass["RoleMainUI"]:refreshAwakePanel()
	end
end

function RoleMgr:refreshPartEmbedPanel( )
	if self.uiClass["RoleMainUI"] ~= nil then
		self.uiClass["RoleMainUI"]:refreshEmbedPanel()
	end
end
