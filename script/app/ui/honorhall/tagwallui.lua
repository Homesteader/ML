...
function TagWallUI:displayByData(data,typeString,uid)
    ...
    for k,hero in pairs(data.info.pos) do
        local heroBaseConf,heroCombatConf,heroModelConf = GlobalApi:getHeroConf(tonumber(hero.hid))
        local obj = ClassRoleObj.new(hero.hid,0)
        obj:setPromoted(hero.promote)
		if heroBaseConf~=nil then
            hero.hid=hero.hid
			hero.pos=k
			hero.icon="uires/icon/hero/" .. heroModelConf.headIcon
			hero.isKing = (tonumber(k)==1) and true or false
            hero.quality=obj:getQuality()
			hero.equips={}
            hero.offsetY = heroModelConf.uiOffsetY
...
function TagWallUI:setHeroView(idx)
	...
	local heroBaseConf,heroCombatConf = GlobalApi:getHeroConf(tonumber(hero.hid))
