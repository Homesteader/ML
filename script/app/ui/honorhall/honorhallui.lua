...
function HonorHallUI:ShowRole(cell,data)
    ...
    local heroModelConf = nil
    if data ~= nil then
        _,_,heroModelConf = GlobalApi:getHeroConf(data.headpic)
    end
    
    if (heroModelConf ~= nil) then
        local nameText = ccui.Helper:seekWidgetByName(cell,"name")
        nameText:setString(data.un)
        local animation  = nil
        local path = heroModelConf.modelUrl
        local promote = nil
        ...
        animation:setPositionY(-10 + heroModelConf.uiOffsetY)
...
function HonorHallUI:createMoveRoles(names)
    ...
    for i = 1,#names do        
       ...
       local _,_,heroModelConf = GlobalApi:getHeroConf(sortRoleDatas[index].id)
       local path = heroModelConf.modelUrl
       ...
       local hpBarHeight = heroModelConf.hpBarHeight
       local size = animation:getContentSize()
       name.hpBarHeight = hpBarHeight
       --name:setPosition(cc.p(0,math.abs(heroModelConf.uiOffsetY * 0.5)))
