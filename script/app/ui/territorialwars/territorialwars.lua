...
local gridRes = {
    selectRes = 'uires/ui/territorialwars/hex_select.png',
    normalRes = 'uires/ui/common/touming.png',
    mainroldRes = 'uires/ui/territorialwars/main_hero.png',
}

local stateImg = {
    occupy = 'uires/ui/territorialwars/occupy.png',   
    plunder = 'uires/ui/territorialwars/plunder.png',
}

local opacityImg = {
    open = 'uires/ui/territorialwars/kai.png',   
    close = 'uires/ui/territorialwars/guan.png',
}

local partimg = {
    red = 'uires/ui/territorialwars/hongdi.png',   
    blue = 'uires/ui/territorialwars/landi.png',
}

local asyncImg = {
    'uires/ui/territorialwars/map_bg.jpg',
    'uires/ui/territorialwars/function_btn.png',
    'uires/ui/territorialwars/material_btn.png',
    'uires/ui/territorialwars/jilu.png',
    'uires/ui/territorialwars/paihang.png',
    'uires/ui/mainscene/hc_nor_btn.png',
    'uires/ui/territorialwars/hex_select.png',
    'uires/ui/territorialwars/hex_normal.png',
    'uires/ui/territorialwars/main_hero.png',
    'uires/ui/territorialwars/red.png',
    'uires/ui/territorialwars/green.png',
    'uires/ui/common/touming.png',
    'uires/ui/territorialwars/footprint.png',
...
function TerritorialWarUI:enter()
    ...
    self.achievetab = {}
    bossBtn:loadTextures('uires/ui/territorialwars/boss.png','','')
    functionBtn:loadTextures('uires/ui/territorialwars/function_btn.png','','')
    materialBtn:loadTextures('uires/ui/territorialwars/material_btn.png','','')
    reportBtn:loadTextures('uires/ui/territorialwars/jilu.png','','')
    rankBtn:loadTextures('uires/ui/territorialwars/paihang.png','','')
    backCity:loadTextures('uires/ui/mainscene/hc_nor_btn.png','','')
    locationBtn:loadTextures('uires/ui/territorialwars/dingwei.png','','')
    mapBtn:loadTextures('uires/ui/territorialwars/map_btn.png','','')
    addbtn:loadTextures('uires/ui/buoy/add_nor_btn.png','','')
    self.footImg = "uires/ui/territorialwars/footprint.png"
    self.flagImg = "uires/ui/territorialwars/flag.png"
    for i=1,3 do
    ...
    mapBtn:setPosition(cc.p(winSize.width - size1.width/2-10,size1.height*2))

    for i=1,3 do
        self.achievetab[i].bg:setPosition(cc.p(winSize.width - size1.width,size.height*5.4 - (i-1)*62))
    end

    self.invadeImg:setPosition(cc.p(winSize.width - size1.width/2-20,size1.height*3-55))
    ...
    helpbtn:setPosition(cc.p(winSize.width - helpSize.width*0.5,winSize.height-helpSize.height/2))
    -- 战报
...
function TerritorialWarUI:loadMapResUIR()
    ...
    for i=1,#dfmapcreatureCfg do
        ...
        local name = monsterModelConf.modelUrl .. "_display"
...
function TerritorialWarUI:createMapRes(mapPos, resInfo)
    ...
    local newRes = ccui.ImageView:create('uires/ui/territorialwars/' .. resType)
    ...
    for i,v in ipairs(leftIds) do
        for j=v,rightIds[i] do
            ...
            if j >= 1 and j <= 25 then
                ...
                if not bgImg then
                    cc.Director:getInstance():getTextureCache():addImageAsync('uires/ui/territorialwars/df_'..j..'.jpg',function(texture)
                        ...
                        local img = ccui.ImageView:create('uires/ui/territorialwars/df_'..j..'.jpg')
...
function TerritorialWarUI:loadMapBlock(pos)
    local newMapBlock = {}
    -- 背景图
    local newBlockBg = ccui.ImageView:create('uires/ui/territorialwars/map_bg.jpg')
...
function TerritorialWarUI:setGateName()
    ...
    for i=1,3 do
        ...
        if pos ~= nil and self.hextab[pos.x]~= nil and self.hextab[pos.x][pos.y] ~= nil then
            ...
            nameLabel:setPosition(cc.p(namebg:getContentSize().width/2,namebg:getContentSize().height/2))
...
function TerritorialWarUI:createMonster(cellId,info,block)
    ...
    local monsterId = fomationConfig['pos'..fomationConfig.boss]
    local monsterConfig,monsterCombatConf,monsterModelConf = GlobalApi:getMonsterConf(monsterId)

    local modelAin = nil
    local bg = block:getChildByTag(cellId + 1000)
    if not bg then
        bg = ccui.ImageView:create('uires/ui/common/touming.png')
        local spineAni = GlobalApi:onlyCreateArmature(monsterModelConf.modelUrl.."_display")
...
function TerritorialWarUI:createMine(cellId,info,block)
    ...
    if not bg then
        ...
        if model~=nil then
            ...
            local namebg = ccui.ImageView:create('uires/ui/legion/building_bg.png')
            namebg:setPosition(cc.p(bg:getContentSize().width/2-45,bg:getContentSize().height+45))
            namebg:setContentSize(cc.size(30, 100))
            namebg:setScale9Enabled(true)

            local richText = xx.RichText:create()
            richText:setContentSize(cc.size(30, 100))

            local height = namebg:getContentSize().height/2-offisize
            ...
            rtImg:setScale(0.9)
            richText:setAnchorPoint(cc.p(0.5,0.5))
            richText:setPosition(cc.p(namebg:getContentSize().width/2,height))
            richText:setAlignment('middle')
...
function TerritorialWarUI:createMode(gridBlock,v,notOpacity)
    ...
    if isEnemy == true then
        image = 'uires/ui/infinitebattle/difficulty_1.png'
    end
    local rtImg = xx.RichTextImage:create(image)
    ...
    for k, v in pairs(fogConf) do
        ...
        if self:isCellCloudShow(k) then
            if self.clouds[k] == nil or self.clouds[k] == 0 then
                local cloud = ccui.ImageView:create('uires/ui/territorialwars/cloud.png')
...
function  TerritorialWarUI:createMapBoss(v,bossCellId)
    ...
    local monsterId = fomationConfig['pos'..fomationConfig.boss]
    local monsterConfig,monsterCombatConf,monsterModelConf = GlobalApi:getMonsterConf(monsterId)
    local scale = monsterModelConf.scale
    local offsetY = monsterModelConf.uiOffsetY
    local modelAin = nil
    local nameOffsetY = 100
    local bg = self.resBlock:getChildByTag(bossCellId + 1000)
    if not bg then
        bg = ccui.ImageView:create('uires/ui/common/touming.png')
        local spineAni = GlobalApi:createLittleLossyAniByName(monsterModelConf.modelUrl.."_display")
        ...
        local bossFlagImg = ccui.ImageView:create('uires/ui/territorialwars/bosshead.png')
