...
function TrainingMainUI:updateSlot(idx)
	...
	if slot ~= nil then
		...
		--slot.trainBtn:setTouchEnabled(not isLock)
			...
			if eventType == ccui.TouchEventType.ended then
				...
				if isTraining==true then
					...
					if self.initMsg['train'][tostring(idx)][3] == 0 then
						if idx == 4 then
							--promptmgr:showSystenHint('需要购买',COLOR_TYPE.RED)
...
function TrainingMainUI:init()
	...
	local re1 = xx.RichTextLabel:create(GlobalApi:getLocalStr('TRAINING_TIPS1'), 23, COLOR_TYPE.ORANGE)
	re1:setStroke(COLOROUTLINE_TYPE.WHITE1, 1)
	--re1:clearShadow()
	...
	local re3 = xx.RichTextLabel:create(GlobalApi:getLocalStr('TRAINING_TIPS2'), 23, COLOR_TYPE.ORANGE)
	re3:setStroke(COLOROUTLINE_TYPE.WHITE1, 1)
	--re3:clearShadow()
