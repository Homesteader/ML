...
function LegionWarCityInfoUI:init()
	...
	local img = "uires/ui/legionwar/" .. cityconf[self.cityid].urlb
	if self.isself == 0 then
		img = "uires/ui/legionwar/" .. cityconf[self.cityid].urlr
	end
...
function LegionWarCityInfoUI:updateDisplay()
	...
	for i = 1, #self.persontab do
		...
		if data.uid ~= 0 then
			...
			local heroBaseConf,heroCombatConf = GlobalApi:getHeroConf(tonumber(userdata.model))
	        ...
	        local _,_,heroModelConf = GlobalApi:getHeroConf(tonumber(userdata.model))
			local changeEquipObj = GlobalApi:getChangeEquipState(promote, weapon_illusion, wing_illusion)
			local hero = GlobalApi:createLittleLossyAniByName(heroModelConf.modelUrl .. "_display", nil, changeEquipObj)
			hero:setScale(0.7)
			hero:getAnimation():play("idle", - 1, - 1)
			hero:setName("hero")
			hero:setPosition(cc.p(78.5, 50 + heroModelConf.uiOffsetY * 0.7))
			self.persontab[index].stateimg:setLocalZOrder(100)
			self.persontab[index].img:addChild(hero)
			local shadow = hero:getBone(heroModelConf.modelUrl .. "_shadow")
			if shadow then
