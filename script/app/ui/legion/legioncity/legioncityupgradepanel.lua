...
function LegionCityUpgradeUI:init()
   ...
   helpBtn:addTouchEventListener(function(sender, eventType)
    ...
    for i = 1, 2 do 
        ...
        local sprite = cc.Sprite:create('uires/ui/legion/cityUpgrade/mask.png')
...
function LegionCityUpgradeUI:refreshAwards(index)
    ...
    for i = 1, 2 do
		...
		local heroBaseConf,heroCombatConf,heroModelConf = GlobalApi:getHeroConf(awardId)
        self.arardArr[i].bg:loadTexture(COLOR_FRAME[heroCombatConf.quality])
        self.arardArr[i].icon:loadTexture('uires/icon/hero/' .. heroModelConf.headIcon)
...
function LegionCityUpgradeUI:refreshCityInfo(index)
    ...
    for i = 1, 4 do
        ...
        if buildLevel == 0 then
            self.effectArr[i].state:loadTexture('uires/ui/legion/cityUpgrade/build_normal.png')
        else
            self.effectArr[i].state:loadTexture('uires/ui/legion/cityUpgrade/upgrade_normal.png')
        end
