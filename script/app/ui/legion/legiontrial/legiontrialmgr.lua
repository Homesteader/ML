...
-- new ~
local ClassLegionTrialMainPannelNewUI = require('script/app/ui/legion/legiontrial/legiontrialmainpannel_new')
local ClassLegionTrialAddRatePannelNewUI = require('script/app/ui/legion/legiontrial/legiontrialaddratepannel_new')
local ClassLegionTrialResetCoinPannelNewUI = require('script/app/ui/legion/legiontrial/legiontrialresetcoinpannel_new')
local ClassLegionTrialGetAwardPannelNewUI = require('script/app/ui/legion/legiontrial/legiontrialgetawardpannel_new')
local ClassLegionTrialAchievementPannelNewUI = require('script/app/ui/legion/legiontrial/legiontrialachievementpannel_new')
local ClassLegionTrialAdventurePannelNewUI = require('script/app/ui/legion/legiontrial/legiontrialadventurepannel_new')
    ...
    local function callBack(serverData)
        if self.uiClass["legionTrialMainPannelUI"] == nil then
		    -- self.uiClass["legionTrialMainPannelUI"] = ClassLegionTrialMainPannelUI.new(serverData,index)
             self.uiClass["legionTrialMainPannelUI"] = ClassLegionTrialMainPannelNewUI.new(serverData,index)
...
-- 刷新
function LegionTrialMgr:refreshTrialMainStatus(data)
    if self.uiClass["legionTrialMainPannelUI"] ~= nil then
        self.uiClass["legionTrialMainPannelUI"]:refreshTrialMainStatus(data)
    end
end
...
function LegionTrialMgr:showLegionTrialGetAwardPannelUI(trial,curChoosePage,callBack)
    if self.uiClass["legionTrialGetAwardPannelUI"] == nil then
        ...
        self.uiClass["legionTrialGetAwardPannelUI"] = ClassLegionTrialGetAwardPannelNewUI.new(trial,curChoosePage,callBack)
...
function LegionTrialMgr:showLegionTrialResetCoinPannelUI(trial,round,index,callBack,callBack2)
    if self.uiClass["legionTrialResetCoinPannelUI"] == nil then
        ...
        self.uiClass["legionTrialResetCoinPannelUI"] = ClassLegionTrialResetCoinPannelNewUI.new(trial,round,index,callBack,callBack2)
...
function LegionTrialMgr:showLegionTrialAchievementPannelUI(trial,callBack)
    if self.uiClass["legionTrialAchievementPannelUI"] == nil then
        ...
        self.uiClass["legionTrialAchievementPannelUI"] = ClassLegionTrialAchievementPannelNewUI.new(trial,callBack)
...
function LegionTrialMgr:showLegionTrialAddRatePannelUI()
    if self.uiClass["legionTrialAddRatePannelUI"] == nil then
        ...
        self.uiClass["legionTrialAddRatePannelUI"] = ClassLegionTrialAddRatePannelNewUI.new()
...
function LegionTrialMgr:showLegionTrialAdventurePannelUI(trial,index)
    ...
    if self.uiClass["legionTrialAdventurePannelUI"] == nil then
        ...
        self.uiClass["legionTrialAdventurePannelUI"] = ClassLegionTrialAdventurePannelNewUI.new(trial,index)
...
function LegionTrialMgr:legionTrialEnterFromServer(callBack)
    local args = {}
    MessageMgr:sendPost("enterTrial", "trial", json.encode(args), function (jsonObj)
...
function LegionTrialMgr:legionTrialExploreFromServer(callBack)
    local args = {}
    MessageMgr:sendPost("explore", "trial", json.encode(args), function (jsonObj)
	    local code = jsonObj.code
	    if code == 0 then
            if callBack then
                callBack(jsonObj.data)
...
function LegionTrialMgr:legionTrialOneExploreFromServer(callBack)
    local args = {}
    MessageMgr:sendPost("explore_round", "trial", json.encode(args), function (jsonObj)
	    local code = jsonObj.code
	    if code == 0 then
            if callBack then
                callBack(jsonObj.data)
...
function LegionTrialMgr:legionTrialResetExploreCoinFromServer(round,index,callBack)
    ...
    MessageMgr:sendPost("reset_explore_coin", "trial", json.encode(args), function (jsonObj)
	    local code = jsonObj.code
	    if code == 0 then
            if callBack then
                callBack(jsonObj.data)
...
function LegionTrialMgr:legionTrialGetExploreAwardFromServer(round,callBack)
    ...
    MessageMgr:sendPost("get_explore_award", "trial", json.encode(args), function (jsonObj)
	    local code = jsonObj.code
	    if code == 0 then
            if callBack then
                callBack(jsonObj.data)
...
function LegionTrialMgr:legionTrialGetAchievementAwardFromServer(achievement_type,achievement_level,callBack)
    ...
    MessageMgr:sendPost("getAchievementAward", "trial", json.encode(args), function (jsonObj)
	    local code = jsonObj.code
	    if code == 0 then
            if callBack then
                callBack(jsonObj.data)
...
function LegionTrialMgr:legionTrialStartChallengeMonsterFromServer(index,callBack)
    ...
    MessageMgr:sendPost("start_challenge_monster", "trial", json.encode(args), function (jsonObj)
	    local code = jsonObj.code
	    if code == 0 then
            if callBack then
                callBack(jsonObj.data)
...
function LegionTrialMgr:legionTrialGetMonsterAwardFromServer(index,callBack)
    local args = {index = index}
    MessageMgr:sendPost("get_monster_award", "trial", json.encode(args), function (jsonObj)
	    local code = jsonObj.code
	    if code == 0 then
            if callBack then
                callBack(jsonObj.data)
...
-- 怪人挑战开始 index 奇遇索引
function LegionTrialMgr:legionTrialBeforeFightFromServer(index,callBack)
    local args = {index = index}
    MessageMgr:sendPost("trial_adventure_before_fight", "trial", json.encode(args), function (jsonObj)
        -- local code = jsonObj.code
        -- if code == 0 then
        --     if callBack then
        --         callBack(jsonObj.data)
...
-- 怪人挑战结束 index 奇遇索引
function LegionTrialMgr:legionTrialFightFromServer(args,callBack)
    MessageMgr:sendPost("trial_adventure_fight", "trial", json.encode(args), function (jsonObj)
        -- local code = jsonObj.code
        -- if code == 0 then
        --     if callBack then
        --         callBack(jsonObj.data)
...
function LegionTrialMgr:legionTrialBuyShopItemFromServer(index,callBack)
    local args = {index = index}
    MessageMgr:sendPost("buy_shop_item", "trial", json.encode(args), function (jsonObj)
	    local code = jsonObj.code
	    if code == 0 then
            if callBack then
                callBack(jsonObj.data)
...
-- 领取每日宝箱
function LegionTrialMgr:legionTrialGetDailyBoxFromServer(callBack)
    local args = {}
    MessageMgr:sendPost("get_daily_award", "trial", json.encode(args), function (jsonObj)
        local code = jsonObj.code
        if code == 0 then
            if callBack then
                callBack(jsonObj.data)
...
-- 领取周成就
function LegionTrialMgr:legionTrialGetWeekAchievementAwardFromServer(id,callBack)
    local args = {index = id}
    MessageMgr:sendPost("get_achievement_award", "trial", json.encode(args), function (jsonObj)
        local code = jsonObj.code
        if code == 0 then
            if callBack then
                callBack(jsonObj.data)
...
-- 怪人星级重置
function LegionTrialMgr:legionTrialResetMonsterStarFromServer(index, callBack)
    local args = {index = index}
    MessageMgr:sendPost("reset_monster_star", "trial", json.encode(args), function (jsonObj)
        local code = jsonObj.code
        if code == 0 then
            if callBack then
                callBack(jsonObj.data)
