...
function LegionLevelsUI:init()
    ...
    awardimg:addTouchEventListener(function (sender, eventType)
        ...
        elseif eventType == ccui.TouchEventType.ended then
            ...
            local tx1 = GlobalApi:getLocalStr('LEGION_LEVELS_DESC6')
            local tx2 = self.data.chapter
...
function LegionLevelsUI:update()
    self.bar:setPercent(LegionMgr:calccopypercent(self.data,self.index))
    self.bartx:setString(LegionMgr:calccopypercent(self.data,self.index)..'%')
    self.cityimg:loadTexture('uires/ui/mainscene/' .. self.legioncopyconf[self.index].icon)
    ...
    for i=1,6 do
        ...
        local monsterInfo,monsterCombatInfo,monsterModelInfo = GlobalApi:getMonsterConf(bossid)

        local monsterurl = 'uires/icon/hero/'..monsterModelInfo.modelUrl .. '_icon.png'
