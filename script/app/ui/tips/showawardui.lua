...
function ShowAwardUI:createAwards(i)
    ...
    local cell = ClassItemCell:create(ITEM_CELL_TYPE.ITEM, award, rootNode)
    ...
    if award:getType() == 'equip' then
        cell.lvTx:setString('Lv.'..award:getLevel())
    else
        cell.lvTx:setString('x'..GlobalApi:toWordsNumber(award:getNum()))
    end
    cell.nameTx:setString(award:getName())
    cell.nameTx:setColor(award:getNameColor())
