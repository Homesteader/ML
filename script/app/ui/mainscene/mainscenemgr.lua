...
local ClassHookMainUI = require("script/app/ui/hook/hookmainui")
local ClassHookDetailUI = require("script/app/ui/hook/hookdetailui")
local ClassHookDispatchUI = require("script/app/ui/hook/hookdispatchui")
local ClassStoreMainUI = require("script/app/ui/mainscene/storemainui")
local ClassStoreEntranceUI = require("script/app/ui/mainscene/storeentranceui")
...
function MainSceneMgr:showDragonInfoUI(id,isHide)
	if self.uiClass['dragonInfoUI'] == nil then
		self.uiClass['dragonInfoUI'] = ClassDragonInfoUI.new(id,isHide)
...
function MainSceneMgr:showHookMainUI(type)
	if self.uiClass['hookMainUI'] == nil then
		local args = {}
		MessageMgr:sendPost('get','auto_fight',json.encode(args),function (response)
			local code = response.code
	        local data = response.data
	        if code == 0 then
				self.uiClass['hookMainUI'] = ClassHookMainUI.new(type,data)
				self.uiClass['hookMainUI']:showUI()
			end
		end)
	end
end

function MainSceneMgr:updateHookMainUI(type,data)
	if self.uiClass['hookMainUI'] then
		self.uiClass['hookMainUI']:update(type,data)
	end
end

function MainSceneMgr:hideHookMainUI()
	if self.uiClass['hookMainUI'] then
		self.uiClass['hookMainUI']:hideUI()
		self.uiClass['hookMainUI'] = nil
	end
end

function MainSceneMgr:showHookDetailUI(lv)

	if self.uiClass['hookDetailUI'] == nil then
		self.uiClass['hookDetailUI'] = ClassHookDetailUI.new(lv)
		self.uiClass['hookDetailUI']:showUI()
			
	end
end

function MainSceneMgr:hideHookDetailUI()
	if self.uiClass['hookDetailUI'] then
		self.uiClass['hookDetailUI']:hideUI()
		self.uiClass['hookDetailUI'] = nil
	end
end

function MainSceneMgr:showHookDispatchUI(taskId)

	if self.uiClass['hookDispatchUI'] == nil then
		self.uiClass['hookDispatchUI'] = ClassHookDispatchUI.new(taskId)
		self.uiClass['hookDispatchUI']:showUI()
			
	end
end

function MainSceneMgr:hideHookDispatchUI()
	if self.uiClass['hookDispatchUI'] then
		self.uiClass['hookDispatchUI']:hideUI()
		self.uiClass['hookDispatchUI'] = nil
	end
end

function MainSceneMgr:showStoreEntranceUI()

	if self.uiClass['storeEntranceUI'] == nil then
		self.uiClass['storeEntranceUI'] = ClassStoreEntranceUI.new()
		self.uiClass['storeEntranceUI']:showUI()			
	end
end

function MainSceneMgr:hideStoreEntranceUI()
	if self.uiClass['storeEntranceUI'] then
		self.uiClass['storeEntranceUI']:hideUI()
		self.uiClass['storeEntranceUI'] = nil
	end
end

function MainSceneMgr:showStoreMainUI(storeType,storeTabId)

	if self.uiClass['storeMainUI'] == nil then
		...
		MessageMgr:sendPost('get','shop',json.encode(args),function (response)
	        ...
	        local data = response.data
	        if code == 0 then
				self.uiClass['storeMainUI'] = ClassStoreMainUI.new(storeType,storeTabId,data.shop)
				self.uiClass['storeMainUI']:showUI()
			else
...
function MainSceneMgr:hideStoreMainUI()

	if self.uiClass['storeMainUI'] then
		self.uiClass['storeMainUI']:hideUI()
		self.uiClass['storeMainUI'] = nil
	end	
end
...
function MainSceneMgr:GetLog()
    ...
    CustomEventMgr:addEventListener("auto_fight_boss_birth",self,function (msg)
    	hookheromgr:ProcessHookBoss(msg)
    end)
end
...
function MainSceneMgr:readAllEmail(allmails, callback)
	local args = {
		-- id = mail.id,
		-- sys = mail.sys
	}

	MessageMgr:sendPost('read_all_mail','user',json.encode(args),function (response)
		...
		if code == 0 then
			...
			for _ , v in ipairs(allmails) do
				if not v.isNew then
					UserData:getUserObj():removeMail(v.arrId)
				end
			end
