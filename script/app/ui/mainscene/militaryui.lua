...
function MilitaryUI:militaryGetWay(id)
	...
	for i=1,len do
		...
		if not desc then
			gotoBtn:setVisible(true)
			if self.lockRTs3[i] then
				self.lockRTs3[i].richText:setVisible(false)
			end
		else
			gotoBtn:setVisible(false)
			if not self.lockRTs3[i] then
				local richText = xx.RichText:create()
				richText:setContentSize(cc.size(450, 30))
				richText:setAlignment('middle')
  				richText:setVerticalAlignment('middle')
				...
				item:addChild(richText)
				self.lockRTs3[i] = {richText = richText,re1 = re1,re2 = re2}
			else
				self.lockRTs3[i].richText:setVisible(true)
			end
		end
	end
