...
function OpenRankUI:rankListPanel()
	...
	for i=2,maxNum do
		...
		if i <= 3 then
			...
			rankImg:loadTexture('uires/ui/report/rank_'..i..'.png')
			rankTx:setString('')
...
function OpenRankUI:updateBottomPanel()
	...
	vipImg:setPosition(cc.p(nameTx:getPositionX() + nameTx:getContentSize().width/2,posY))
	...
	local changeEquipObj = GlobalApi:getChangeEquipState(self.data[RANK_TOP_STYPE[self.page]].promote[1],
		self.data[RANK_TOP_STYPE[self.page]].weapon_illusion,self.data[RANK_TOP_STYPE[self.page]].wing_illusion)
	self.eSpineAni = GlobalApi:createLittleLossyAniByRoleId(tonumber(self.data[RANK_TOP_STYPE[self.page]].model), changeEquipObj)
	self.eSpineAni:setPosition(cc.p(180,120))
	self.bottomImg:addChild(self.eSpineAni)
...
function OpenRankUI:updateTopPanel()
	...
	local str = ''
	if self.page == 2 then
		str = GlobalApi:getLocalStr('OPEN_RANK_DESC_1')
	end
	local conf = GameData:getConfData('avrank')[self.page]
	...
	local str3 = GlobalApi:getLocalStr('OPEN_RANK_DESC_3')..str
	if not self.rts then
		...
		re3:setFont('font/gamefont1.TTF')
