...
function AltarMainUI:setUIBackInfo()
    UIManager.sidebar:setBackBtnCallback(UIManager:getUIName(GAME_UI.UI_ALTARMAINUI), function (sender, eventType)
...
function AltarMainUI:init()
    ...
    --     end
    -- end)
    -- closebtn:setPosition(cc.p(8,winsize.height - 8))
    closebtn:setVisible(false)

    -- local closebtnSize  = closebtn:getContentSize()

    local titleBgImg    = self.root:getChildByName('title_img')
    -- local titleNameTx   = titleBgImg:getChildByName('title_name_tx')

    titleBgImg:setVisible(false)

    -- titleNameTx:setString( UIManager:getUIName(self.uiIndex) )

    -- titleBgImg:setPosition(cc.p(closebtnSize.width + 20,winsize.height))
    ...
    for i = 1, 4 do
        ...
        infodesc1:setString(GlobalApi:getLocalStr_new('ATLAR_DESC2'))

        arr.noopenimg   = bg:getChildByName('no_open_img')
        arr.desctx      = arr.noopenimg:getChildByName('desc_tx')

        local infobg2   = arr.pl:getChildByName('info_bg2')
        local infodesc2 = infobg2:getChildByName('desc_tx2')

        infodesc2:setString(GlobalApi:getLocalStr_new('ATLAR_DESC1'))
        ...
        arr.fastbtntx:setString(GlobalApi:getLocalStr_new('AUTO_ALTAR'))

        arr.paraybtn    = arr.pl:getChildByName('func_btn')

        arr.fastBtn:setVisible(false)
        ...
        --             promptmgr:showSystenHint(GlobalApi:getLocalStr_new('ATLAR_DESC5'), COLOR_TYPE.RED)
        --             return
        --         else
        ...
        --                 self:sendAutoMsg(i,bg,num1,num*cost,string.format(GlobalApi:getLocalStr_new('AUTO_ALTAR_COST'),num*cost,
        ...
        arr.paraybtn:addTouchEventListener(function (sender, eventType)
            if eventType == ccui.TouchEventType.ended then
                ...
                if count >= vipconf[tostring('altar'..i)]  then
                    ...
                    if count+1 > self.altarconf[i].free then
                        local str = string.format(GlobalApi:getLocalStr_new('ATLAR_DESC4'),cost)
...
function AltarMainUI:update()
    ...
    for i=1,4 do
        ...
        self.praytabel[i].paraybtntx:setString(GlobalApi:getLocalStr_new('ATLAR_BTNTX'..i))
        self.praytabel[i].paraybtntx:setFontSize(28)
        self.praytabel[i].iconimg:loadTexture('uires/ui_new/altar/'..self.altarconf[i].bg)

        if i == 3 then
            ShaderMgr:setGrayForWidget(self.praytabel[i].iconimg)
        end
        ...
        if self.altarconf[i]['award'] and self.altarconf[i]['award'][1] then
            ...
            self:initState(self.praytabel[i].titlename,i,num,icon)
        else
            self:initState(self.praytabel[i].titlename,i,nil,nil)
        end
        local award         = DisplayData:getDisplayObj(self.altarconf[i]['award'][1])
        local levelConf     = GameData:getConfData('level')[UserData:getUserObj():getLv()]
        ...
        if altar and altar[tostring(i)] then
            ...
            if count >= self.altarconf[i].free then
                ...
                self.praytabel[i].costtx:setString(GlobalApi:getLocalStr_new('STR_FREE'))
            end
        else
            self.praytabel[i].numTx:setString(levelConf['altar'..i])
...
function AltarMainUI:initState(node,index,num,awardIcon)
    ...
    if UserData:getUserObj():getLv() < self.altarconf[index].level then
        -- self.praytabel[index].titlename:removeAllChildren()
         local str =string.format(GlobalApi:getLocalStr_new('STR_POSCANTOPEN1'),self.altarconf[index].level)
        ...
        self.praytabel[index].noopenimg:setVisible(true and (str ~= ''))
        ...
        self.praytabel[index].noopenimg:setVisible(false)
    ...
    node:setString(GlobalApi:getLocalStr_new('ATLAR_TITLE'..index))
