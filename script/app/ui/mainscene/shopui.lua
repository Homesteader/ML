...
function ShopUI:getIconRes(id)
    ...
    elseif id == 'gold' then
        tab.icon = 'uires/ui/res/gold.png'
    ...
    if self.page == SHOP_TYPE.LISTSHOP or self.page == SHOP_TYPE.TOWERSHOP or self.page == SHOP_TYPE.LEGIONDRESSSHOP 
        ...
        if freeTimes <= 0 then
            if token > 0 then
                ...
                szImg:loadTexture('uires/ui/res/cash.png')
            end
...
function ShopUI:init()
    ...
    self.shopImg = bgImg:getChildByName("shop_img")
    self:adaptUI(bgImg, self.shopImg)
    local winSize = cc.Director:getInstance():getVisibleSize()
    self.shopImg:setPosition(cc.p(winSize.width/2,winSize.height/2 - 40))
    ...
    self.neiBgImg = self.shopImg:getChildByName("nei_bg_img")
    self.cellSv = self.neiBgImg:getChildByName('cell_sv')
    ...
    local function btnInit(min,max)
        if min == max then
            local btn = self.shopImg:getChildByName('page_'..min..'_btn')
            ...
            for i = min,max do
                local btn = self.shopImg:getChildByName('page_'..i..'_btn')
                local newImg = ccui.ImageView:create('uires/ui/common/new_img.png')
                newImg:setPosition(cc.p(165,48))
                newImg:setName('new_img')
                newImg:setVisible(false)
