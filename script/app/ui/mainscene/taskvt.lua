...
local nobilityIconUrl = "uires/ui/worldwar/"
local lightStarUrl = {
	light = "uires/ui/common/icon_star3.png",
	dark = "uires/ui/common/icon_star3_bg.png",
}
local upImg = "uires/ui/task/jinji.png"
...
function TaskNewUI:getMrData()
	...
	for i=1,#conf do
		...
		if status ~= 1 and times >= (conf[i].target or 0) then 		--可领取
			status = 0
			self.cangetCount = self.cangetCount + 1
		elseif status ~= 1 and times < (conf[i].target or 0) then 	--未达成
			status = 1
		else 														--已领取
			status = 2
		end
		
		local key = conf[i].key
		local routetoCfg = GameData:getConfData("routeto")[key]
		local desc,isOpen = nil,0
    	if routetoCfg then
    		if routetoCfg.value == 2 then
				desc = GlobalApi:getGotoLegionModule(routetoCfg.key,true)
				if not desc then
					isOpen = 0
				end
			elseif routetoCfg.value == 1 then
...
function TaskNewUI:sortNewData()
...
function TaskNewUI:updateMR()
	...
	for i,v in ipairs(self.newData) do
		...
		nameTx:setString(getdesc(conf[v.id].event))
		...
		nameTx1:setColor(fontColor)
		nameTx1:enableOutline(outColor, 1)
		nameTx:setColor(fontColor)
		nameTx:enableOutline(outColor, 1)

		if conf[v.id].target then
			local x = ((times > conf[v.id].target) and conf[v.id].target) or times
			local tx2 = '('..x..'/'..conf[v.id].target..')'
			nameTx1:setString(tx2)
			if times >= conf[v.id].target then
				nameTx1:setColor(COLOR_TYPE.GREEN)
				nameTx1:enableOutline(COLOROUTLINE_TYPE.GREEN, 1)
			end
		end
			...
			if status == 0 then
				...
				getBtn:addTouchEventListener(function (sender, eventType)
			        ...
			        elseif eventType == ccui.TouchEventType.ended then
						...
						local getTab = {}
						for k,v in pairs(self.newData) do
							if v.status == 0 then
								--[[if conf[v.id].key == 'food' then
									local food = UserData:getUserObj():getFood()
									local maxFood = tonumber(GlobalApi:getGlobalValue('maxFood'))
