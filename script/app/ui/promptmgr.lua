...
function promptmgr:showMessageBox(str, mbtype, okfunc, oktext, canceltext, cancelfunc, noLongerShow)
	...
	if self.messageboxNode == nil then
		...
		-- 	self:hideMessageBox()
		-- end)
		local titleTx = messageboxImg:getChildByName('title_tx')
		titleTx:setString('')
	...
	if type(str) == 'string' then
		...
		if not msg then
			msg = cc.Label:createWithTTF('', 'font/gamefont.ttf', 22)
			msg:setAnchorPoint(cc.p(0.5, 0.5))
			msg:setPosition(cc.p(193, 169))
			msg:setMaxLineWidth(424)
			msg:setHorizontalAlignment(cc.TEXT_ALIGNMENT_LEFT)
			msg:setColor(COLOR_TYPE.WHITE1)
			msg:enableOutline(COLOROUTLINE_TYPE.OFFWHITE1,2)
...
-- cancelfunc 退出按钮回调
-- noLongerShow 不再提示
function promptmgr:showMessageBoxWithTitle(titleStr, str, mbtype, okfunc, oktext, canceltext, cancelfunc, noLongerShow)
