...
function LoginUI:createSelectQu()
	...
	for i=1,3 do
		...
		local serverTx = roleBgNode:getChildByName('server_tx')
    ...
    if not isHide then
		self.root:runAction(cc.Sequence:create(cc.DelayTime:create(0.5),
			cc.CallFunc:create(function()
				...
				self.noticeBg:runAction(cc.FadeIn:create(0.5))
...
function LoginUI:initLoading()
    local winSize = cc.Director:getInstance():getVisibleSize()
    local loadingBarBg = cc.Sprite:create("uires/ui/common/bar_bg_1.png")
    local size = loadingBarBg:getContentSize()
    local loadingBar = ccui.LoadingBar:create("uires/ui/common/bar_1.png")
    -- local loadingBar = ccui.LoadingBar:create(cc.Sprite:create("uires/ui/common/bar_1.png"))
    ...
    numTx2:setString(GlobalApi:getLocalStr_new('LOGIN_DESC'))

	loadingBarBg:setPosition(winSize.width/2,85)
	self.root:addChild(loadingBarBg)

    local nowImgCount = 8
    local loadedImgMaxCount = 38
        ...
        self.root:runAction(cc.Sequence:create(cc.DelayTime:create(0.1), cc.CallFunc:create(function ()
            ...
            if not flag then
		 		...
		 		MainSceneMgr:showMainCityFromLogin()
		 	 	local confIndex = MapData:getCurProgress()
				if confIndex > 0 then
    ...
    cc.Director:getInstance():getTextureCache():addImageAsync('uires/ui_new/buoy/fight_nor_btn.png',imageLoaded)
    cc.Director:getInstance():getTextureCache():addImageAsync('uires/ui_new/buoy/fight_sel_btn.png',imageLoaded)
    ...
    cc.Director:getInstance():getTextureCache():addImageAsync('uires/ui_new/maincity/camp.png',imageLoaded)

    GlobalApi:createSpineAsyncByName('mainscene_animal_1','spine/mainscene_animal_1/mainscene_animal_1', imageLoaded)
    GlobalApi:createSpineAsyncByName('mainscene_animal_2','spine/mainscene_animal_2/mainscene_animal_2', imageLoaded)
    GlobalApi:createSpineAsyncByName('mainscene_animal_3','spine/mainscene_animal_3/mainscene_animal_3', imageLoaded)
    ...
    cc.Director:getInstance():getTextureCache():addImageAsync('uires/ui/maincity/statue_tx_img.png',imageLoaded)
    cc.Director:getInstance():getTextureCache():addImageAsync('uires/ui/buoy/new_point.png',imageLoaded)
    cc.Director:getInstance():getTextureCache():addImageAsync('uires/ui/guard/lock.png',imageLoaded)
    ...
    GlobalApi:createSpineAsyncByName('shop','spine/city_building/shop', imageLoaded)
    GlobalApi:createSpineAsyncByName('worldwar','spine/city_building/worldwar', imageLoaded)
    GlobalApi:createSpineAsyncByName('train','spine/city_building/train', imageLoaded)
    ]]
...
function LoginUI:init()
    ...
    local logo = self.root:getChildByName("logo_img")
    logo:setTexture('loginpanel/logo.png')
    logo:setAnchorPoint(cc.p(0, 1))
    logo:setPosition(cc.p(20, winSize.height - 20))
	...
	enterGameBtn:addTouchEventListener(function (sender, eventType)
		...
		elseif eventType == ccui.TouchEventType.ended then
			...
			self.noticeBg:setVisible(false)
			self.noticeBtn:setTouchEnabled(false)
			self:initLoading()
		end
	end)
    ...
    self.noticeBg = self.root:getChildByName("notice_bg")
    self.noticeBtn = self.noticeBg:getChildByName("notice_btn")
	self.noticeBg:setOpacity(0)
	self.noticeBtn:setTouchEnabled(false)
    ...
    self.noticeBg:setAnchorPoint(cc.p(1,1))
    self.noticeBg:setPosition(cc.p(winSize.width - 10,winSize.height - 10))
	...
	self.logoutBg:setVisible(Third:Get():isShowLogoutBtn())
...
function LoginUI:downloadNewClient()
	...
	local okTx = okBtn2:getChildByName('info_tx')
	okTx:setString(GlobalApi:getLocalStr("STR_OK2"))
	okBtn2:addTouchEventListener(function (sender, eventType)
		if eventType == ccui.TouchEventType.began then
            AudioMgr.PlayAudio(11)
        elseif eventType == ccui.TouchEventType.ended then
        	local url = GlobalData:getDownloadUrl()
			Third:Get():openUrl(url)
        end
	end)
...
function LoginUI:createAME()
	...
	local loadingBarBg = cc.Sprite:create("uires/ui/common/bar_bg_1.png")
	local size = loadingBarBg:getContentSize()
	local loadingBar = cc.ProgressTimer:create(cc.Sprite:create("uires/ui/common/bar_1.png"))
	local barSize = loadingBar:getContentSize()
	loadingBar:setType(cc.PROGRESS_TIMER_TYPE_BAR)
	loadingBar:setMidpoint(cc.p(0, 0))
	loadingBar:setBarChangeRate(cc.p(1, 0))
	...
	local manifest = "resource/manifest/project.manifest"
	...
	local listener
	local errorCount = 0
	local showDownloadSize = false
	local function updateDownloadedSize()
		if showDownloadSize then
	...
	local function onUpdateEvent(event)
		...
		elseif  eventCode == cc.EventAssetsManagerEx.EventCode.UPDATE_PROGRESSION then
			...
			if assetId ~= cc.AssetsManagerExStatic.VERSION_ID and assetId ~= cc.AssetsManagerExStatic.MANIFEST_ID then
				loadingBar:setPercentage(event:getPercentByFile())
				local num = string.format("%d", event:getPercentByFile())
		...
		elseif eventCode == cc.EventAssetsManagerEx.EventCode.UPDATE_FAILED then
			if errorCount == 0 then
				...
				promptmgr:showMessageBox(errorCount .. GlobalApi:getLocalStr("ERROR_UPDATE"), MESSAGE_BOX_TYPE.OK, function ()
					loadingBar:setPercentage(0)
					numTx1:setString("0%")
					am:downloadFailedAssets()
				end)
