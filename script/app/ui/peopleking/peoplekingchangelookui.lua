...
function PeopleKingChangeLookUI:init()
    ...
    local titleTx       = bg_img1:getChildByName("title_tx")
    titleTx:setString(GlobalApi:getLocalStr_new("STR_PEOPLEKING_DESC_9"))

    self.pageBtns       = {}

    for i = 1, 2 do
        local pageBtn           = bg_img1:getChildByName("page_"..i.."_btn")
    ...
    --列表信息
    self.cardSv                 = leftBg:getChildByName("sv")
...
function PeopleKingChangeLookUI:collectInfo(showAttrTip)
    ...
    local haveTx    = self.bottombg:getChildByName("have_num_tx")
    local preStr    = ''

    if self.page == 1 then
        preStr      = string.format( GlobalApi:getLocalStr_new('STR_PEOPLEKING_DESC_12'), tostring(value))
    else
        preStr      = string.format( GlobalApi:getLocalStr_new('STR_PEOPLEKING_DESC_13'), tostring(value))
    end
    descTitle:setString( preStr )
    haveTx:setString('(' .. value .. '/' .. goalValue .. ')')
    ...
    for i = 1, 2 do
        ...
        local attrValueTx   = attrNameTx:getChildByName("attr_value_tx"..i)

        attrNameTx:setString(attrNameStr)
        attrValueTx:setString("+"..attrvalue.."%")
        attrValueTx:setPositionPercent( cc.p(1, 0.5) )

        local allAttrNameTx     = self.bottombg:getChildByName("all_attr"..i)
        local allAtterValueTx   = allAttrNameTx:getChildByName("all_attrvalue"..i)
        ...
        local str               = string.format(GlobalApi:getLocalStr("PEOPLE_KING_CHANGELOOK_DESC_16"),'')

        allAttrNameTx:setString(str)
        allAtterValueTx:setString("+"..allValue.."%")
        allAtterValueTx:setPositionPercent( cc.p(1, 0.5) )
...
function PeopleKingChangeLookUI:showCollectItem()
    ...
    self.selectImg          = ccui.ImageView:create('uires/ui_new/common/head_select.png')
    self.selectImg:setName("selectimg")
    self.selectImg:setScale(1)
    ...
    for i = 2, 1,-1 do
        ...
        local plheight      = 95* (math.ceil(#itemCfg / lineNum))
        ...
        nametx:setPosition(cc.p(10,svheight-15))
        ...
        for j = 1, #itemCfg do
            ...
            local x = 45 + 90 * ( (j-1) % lineNum )
            local y = plheight - 45 - 90 * (math.ceil(j / lineNum) - 1)
...
function PeopleKingChangeLookUI:updateRigthInfo(ntype,index)
    ...
    self.attrTxR:setString(string.format(str,'',attrValue).."%")
    self.attrTxR:setVisible(attrValue ~= 0)
    ...
    cashIcon:loadTexture('uires/ui_new/res/cash.png')
    self.getwayDesc:setVisible(false)
    ...
    if isActive then
        ...
        if conditionType == "buy" then
            ...
            infoTx:setString("")
            self.getwayDesc:setVisible(true)
            local tx = self.getwayDesc:getChildByName('getway_desc_tx')
            tx:setString(desc)
        end
    end
