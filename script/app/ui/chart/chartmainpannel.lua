...
function ChartMainPannelUI:updateEveryItem(i)
    ...
    if demoData[i] > 0 then
        local heroBaseConf,heroCombatConf,heroModelConf = GlobalApi:getHeroConf(demoData[i])
        ...
        self.frames[i].icon:loadTexture('uires/icon/hero/' .. heroModelConf.headIcon)
...
function ChartMainPannelUI:start()
    ...
    for k,v in pairs(self.cells) do
        if v then
            local _,_,heroModelConf = GlobalApi:getHeroConf(v.id)
            v.heroImg:loadTexture('uires/icon/big_hero/' .. heroModelConf.bodyIcon)
...
function ChartMainPannelUI:initData()
    ...
    for k,v in pairs(conf) do
        local _,heroCombatConf = GlobalApi:getHeroConf(v.id)
        if tempDataTab[heroCombatConf.camp] and heroCombatConf.quality >= 4 then
            local index = #tempDataTab[heroCombatConf.camp] + 1
            tempDataTab[heroCombatConf.camp][index] = v
            if roleCardMap[v.id] then -- 这里是只要存在，不管分不分解
                tempDataTab[heroCombatConf.camp][index].hasRole = true
                self.numTab[heroCombatConf.camp].hasNum = self.numTab[heroCombatConf.camp].hasNum + 1
            end
        end
    end
    ...
    for i = 1,4 do
        ...
        local temp2 = {}
        for j = 1,#temp do
            local _,heroCombatConf = GlobalApi:getHeroConf(temp[j].id)
            if heroCombatConf.quality == 4 then
                temp1[#temp1 + 1] = temp[j]
            elseif heroCombatConf.quality == 5 then
                temp2[#temp2 + 1] = temp[j]
            elseif heroCombatConf.quality == 6 then
...
function ChartMainPannelUI:updateCell(celltype)
    ...
    for i = 1,#cureDataTab do
        ...
        card.bigIcon = heroModelConf.bodyIcon
        card.hasRole = cardData.hasRole

        card:loadTexture(COLOR_CARDBG[heroCombatConf.quality] or COLOR_CARDBG[1])
        if cardData.hasRole then
            heroImg:loadTexture('uires/icon/big_hero/' .. heroModelConf.bodyIcon)   
