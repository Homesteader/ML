...
function ChartInfoUI:ctor(cardData,showType,obj)
    ...
    if obj then
        ...
        --]]
        self.obj = RoleData:getRoleInfoById(obj:getId())
        self.cardData = GlobalApi:getHeroConf(obj:getId())
    else
...
function ChartInfoUI:init()
    ...
    local soliderImg = left_bg:getChildByName("solider_img")
    local soliderId = cardData.soldierId or 1
    soliderImg:loadTexture('uires/ui/common/'..'soldier_'..soliderId..'.png')

    local titleText = left_bg:getChildByName("title_text")
    ...
    local _,heroCombatConf = GlobalApi:getHeroConf(cardData.id)
    countryImg:loadTexture('uires/ui/country/flag_'..heroCombatConf.camp..'.png')

    local chartImg = anmPl:getChildByName("chart_img")
    chartImg:ignoreContentAdaptWithSize(true)
    chartImg:loadTexture('uires/ui/tavern/tujian.png')
    ...
    local _,heroCombatConf,heroModelConf = GlobalApi:getHeroConf(cardData.id)
    local spineAni = GlobalApi:createLittleLossyAniByName(heroModelConf.modelUrl .. "_display")
	if spineAni then
	--  spineAni:setScale(0.6)
		local shadow = spineAni:getBone(heroModelConf.modelUrl .. "_shadow")
		if shadow then
			shadow:changeDisplayWithIndex(-1, true)
		end
		local effectIndex = 1
		repeat
			local aniEffect = spineAni:getBone(heroModelConf.modelUrl .. "_effect" .. effectIndex)
		...
		until false
		spineAni:setPosition(cc.p(anmPl:getContentSize().width/2,60+heroModelConf.uiOffsetY))
    ...
    if self.showType == ROLE_SHOW_TYPE.NORMAL then
        anmPl:setPositionY(anmPl:getPositionY() - 30)
        probgImg:setVisible(false)
    ...
    elseif self.showType == ROLE_SHOW_TYPE.CARD_DECOMPOSE then
		...
		local re3 = xx.RichTextImage:create('uires/ui/res/soul.png')
		re3:setScale(0.8)
		...
		richText:setPosition(cc.p(170, 53))
...
function ChartInfoUI:genSkill(height)
    ...
    if self.cardData.quality == 5 then
        local promoteSkillBtn = ccui.ImageView:create('uires/ui/honorhall/fengjiangjineng.png')
...
function ChartInfoUI:getSkillIdTab()
	local skilltab ={}
    local _,heroCombatConf = GlobalApi:getHeroConf(self.cardData.id)
	local skillgroup = heroCombatConf.skillGroupId[1]
...
function ChartInfoUI:genTitle(idx)
	...
	local title = cc.Label:createWithTTF(titletx, "font/gamefont1.TTF", 28)
...
function ChartInfoUI:calFate()
	...
	local fatearr = {}
    local heroBaseConf,heroCombatConf,heroModelConf = GlobalApi:getHeroConf(self.cardData.id)
	fatearr = heroCombatConf.fateGroup
	local fateconf = GameData:getConfData('fate')
