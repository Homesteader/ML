...
local function FileSaveLoad(jsonData)
	if cc.Application:getInstance():getTargetPlatform() == kTargetWindows then
		local file = io.open("D:\\last.login_xunl2.json", "w")
		assert(file)
...
function UserData:initWithData(jsonData)
	...
	self.userObj:setInfinite(jsonData.data.unlimited)
    ...
    self.userObj:setTrial(jsonData.data.trial)
    self.userObj:setHookBoss(jsonData.data.boss_birth)
	MapData:initWithData(jsonData.data.battle)
	RoleData:init()
    self.userObj:addGlobalTime()
    self.userObj:setSocketMailStatus(false)
    self.userObj:setDefaultRefineItem()
end
