...
-- 获取可以合成多少个指定类型指定等级的宝石
-- 返回：可以合成的宝石数量，以及消耗列表
function BagData:getGemComposeData(gemType, gemLevel)
    if gemLevel <= 1 then
        return 0,nil
    end

    self.gemComposeResult = {}

    local consumeList = {}
    local preLevelGemCount = 0;
    for i = 1, gemLevel - 1 do
        local gid = gemType * 100 + i
        local gemObj = self:getGemById(gid)

        local nextLevelGemCount = 0
        --if gemObj then
            local hasNum = 0
            if gemObj then
            ...
            nextLevelGemCount = math.floor((hasNum + preLevelGemCount) / 3)

            if nextLevelGemCount > 0 then
                local consumeObj = {}
                consumeObj.level = i
                consumeObj.num = nextLevelGemCount * 3 - preLevelGemCount
                table.insert(consumeList, consumeObj)
            end

            local resultObj = {}
            resultObj.level = i + 1;
            resultObj.num = nextLevelGemCount
            table.insert(self.gemComposeResult, resultObj)

            preLevelGemCount = nextLevelGemCount
        --end
    end

    return preLevelGemCount, consumeList
end

-- 获取合成本级宝石可以合成多少个
function BagData:getGemComposeCount(gemLevel)
    for k,v in pairs(self.gemComposeResult) do
        if v.level == gemLevel then
            return v.num
        end
    end
...
-- 计算合成指定等级的宝石的消耗列表
function BagData:getGemCompostConsumeList(gemType, gemLevel, gemNum)
    local gemConf = GameData:getConfData("gem")
    local startLevel = gemLevel - 1

    local consumeList = {}
    local needNum = GEMUPGRADENEEDNUM * gemNum

    for i = startLevel,1,-1 do
        local hasNum, existGemObj = BagData:getGemNumByTypeAndLevel(gemType, i)
        ...
        if hasNum >= needNum then
            ...
            -- 计算出缺少量
            local lackCount = needNum - hasNum
...
-- 获取指定类型、等级的宝石数量
function BagData:getGemNumByTypeAndLevel(gemType, gemLevel)
    local retNum = 0

    if self.gemMap[gemType] then
        for k, v in pairs(self.gemMap[gemType]) do
            if v:getLevel() == gemLevel then
                return v:getNum(), v
            end
        end
    end
