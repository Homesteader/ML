...
-- 获取上阵武将数量
function RoleData:getFightRoleNum()
	local cnt = 0
...
function RoleData:isFateActive(fid, role)
	...
	local isActive = true
	for i=1,5 do
		local hid = tonumber(conf['hid' .. i])
		local heroBaseConf = GlobalApi:getHeroConf(id)
		if hid > 0 and roleCardMap[hid] == nil  then
			isActive = false
			break
		end
	end
...
function RoleData:getFateAtt(obj)
	...
	if obj:getId() > 0 then
		 ...
		 for i=1,#fateatt do
		 	...
		 	local atttab1 = fateconf[tonumber(fateatt[i].fateid)]['att11']
			local attvalue = fateconf[tonumber(fateatt[i].fateid)]['value11']
			local atttab2 = fateconf[tonumber(fateatt[i].fateid)]['att12']
			local attvalue2 = fateconf[tonumber(fateatt[i].fateid)]['value12']
			if atttab1 then
				for i=1,#atttab1 do
...
function RoleData:CalPosFetterActive(obj,isonlyforcalc)
	local innateconf = GameData:getConfData('innate')
	local innateGroupconf = GameData:getConfData('innategroup')
	local rolePosFetterAllMap = {}  --全体有效的羁绊属性
	local rolePosFetterMap = {}	    --各个位置羁绊属性
	for i=1,MAXROlENUM do
		local objtemp = self:getRoleByPos(i)
		if objtemp:getId() > 0   then
			local _,heroCombatConf = GlobalApi:getHeroConf(objtemp:getId())
			local att = {}
			local atttemp = {}
			local innatearr = objtemp:getObjfetterAtt()
			for j=1,#innatearr do
				local groupid = heroCombatConf.innateGroup
				local innateid = tonumber(innateGroupconf[groupid]['teamId' .. j])
			...
			rolePosFetterAllMap[i] = atttemp
			rolePosFetterMap[i] = att
		end
	end
	return rolePosFetterAllMap ,rolePosFetterMap
end

--计算上阵羁绊属性
function RoleData:getfetterAtt(obj,isonlyforcalc)
	...
	local rolePosFetterAllMap,rolePosFetterMap = self:CalPosFetterActive(obj,isonlyforcalc)
	local innateposatt = rolePosFetterMap[obj:getPosId()]
	local inateallposatt = rolePosFetterAllMap

	local heroId = obj:getId()
	...
	for j=1,#inateallposatt do
		local sigleAttr = inateallposatt[j]
		if sigleAttr then
			local _,herCombatConf = GlobalApi:getHeroConf(self:getRoleByPos(j):getId())
			for i=1,#inateallposatt[j] do
				local groupid = herCombatConf.innateGroup
				local innateid = tonumber(innateGroupconf[groupid]["teamId"..i])
				local innateaddtype = tonumber(innateconf[innateid].type)
				local id1 = tonumber(sigleAttr[i][1])
				local effect1 = tonumber(sigleAttr[i][2])
				local id2 = tonumber(sigleAttr[i][3])
				local effect2 = tonumber(sigleAttr[i][4])
				if innateaddtype == 1 then
					if id1 > 0 then
						attnum[id1] = attnum[id1] + effect1
					end
...
function RoleData:getInnateAtt(obj,isonlyforcalc)
	...
	local _,heroCombatConf = GlobalApi:getHeroConf(obj:getId())
	local groupid = heroCombatConf.innateGroup
...
function RoleData:CalPosAttByPos(obj,isonlyforcalc)
	...
	if obj:getId() > 0  then
		...
		--上阵羁绊属性
		local fetternum , fetterpercent = self:getfetterAtt(obj,isonlyforcalc)
		for i=1, self.attcount do
			att[i] =  att[i] + math.floor(baseattarr[i]*(fetterpercent[i]/100))
			att[i] =  att[i] + fetternum[i]
		end
        ...
        -- 部位镶嵌属性
        local embedAttr = obj:getEmbedAttr()
        for i=1,self.attcount do
			att[i] =  att[i] + (embedAttr[i] or 0)
		end

        -- 部位觉醒属性
        local awakeAttr = obj:getAwakeAttr()
        for i=1,self.attcount do
			att[i] = att[i] + (awakeAttr[i] or 0)
		end
	end
