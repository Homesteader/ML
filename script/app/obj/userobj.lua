...
--	honor = 0,  --幸运值
	...
	tavern = {
		...
		nfree = 0,			--已使用普通招募免费招募次数
		hcash = 0,			--高级招募是否使用过招募令
		ncall = 0,			--普通招募次数
		hcall = 0,			--普通招募次数
    	hot_time = 0,		--兑换英雄上次刷新时间
    	hid = 0,			--兑换英雄id
    	gold_count = 0,		--金币招募次数
	},
	rescopy = {
...
function UserObj:getWarCollegeSign()
    ...
    if progress < #conf then
    	local cityProgress = MapData:getCurProgress()
...
function UserObj:initUserTavern( json )
	self.tavern.ntime = json.ntime or 0						--普通招募下次免费招募时间
	self.tavern.htime = json.htime or 0						--高级招募下次免费招募时间
	self.tavern.nfree = json.nfree or 0						--已使用普通招募免费招募次数
	self.tavern.hcash = json.hcash or 0						--高级招募是否使用过招募令
	self.tavern.ncall = json.call_count["1"] or 0				--普通招募次数
	self.tavern.hcall = json.call_count["2"] or 0				--高级募次数
    self.tavern.hot_time = json.hero_refresh_time or 0		--兑换英雄上次刷新时间
    self.tavern.hid = json.exchange_hid or 0				--兑换英雄id
    self.tavern.gold_count = json.gold_count or 0			--金币招募次数
end
...
--兑换英雄上次刷新时间
function UserObj:getTavenHotTime()
	return self.tavern.hot_time
end

function UserObj:setTavenHotTime(time)
	self.tavern.hot_time = time or self.tavern.hot_time
end

--兑换英雄
function UserObj:getTavenHotHero()
	return self.tavern.hid
end

function UserObj:setTavenHotHero(hid)
	self.tavern.hid = hid or self.tavern.hid
end

--普通免费招募次数
function UserObj:getTavenNormalFree()
	return self.tavern.nfree
end

function UserObj:setTavenNormalFree(nfree)
	self.tavern.nfree = nfree or self.tavern.nfree
end

--下次普通招募免费时间
function UserObj:getTavenNextNormalTime()
	return self.tavern.ntime
end

function UserObj:setTavenNextNormalTime(ntime)
	self.tavern.ntime = ntime or self.tavern.ntime
end

--下次高级招募免费时间
function UserObj:getTavenNextHighTime()
	return self.tavern.htime
end

function UserObj:setTavenNextHighTime(htime)
	self.tavern.htime = htime or self.tavern.htime
end

--高级招募总次数
function UserObj:getTavenHighCallCnt()
	return self.tavern.hcall
end

function UserObj:setTavenHighCallCnt(cnt)
	self.tavern.hcall = cnt or self.tavern.hcall
end

--金币招募次数
function UserObj:getTavenGoldCallCnt()
	return self.tavern.gold_count
end

function UserObj:setTavenGoldCallCnt(goldCnt)
	self.tavern.gold_count = goldCnt or self.tavern.gold_count
end
...
function UserObj:updateLv()
	...
	if Third:Get():needPostUserLevelUpGameLogBySDK() and levelUpFlag then
		local roleName = tostring(self:getName())
		if roleName == "" then
			roleName = tostring(self:getUid())
		end
		local obj = {
			roleId = tostring(self:getUid()),
			roleName = roleName,
			roleLevel = tostring(self:getLv()),
			serverId = tostring(GlobalData:getSelectSeverUid()),
			serverName = tostring(GlobalData:getSelectSeverName()),
			roleCTime = tostring(self:getCreateTime()),
			roleLevelMTime = tostring(GlobalData:getServerTime())
		}
...
function UserObj:runFightforce(fightLabel,plId)
	...
	if self.oldFightForce and fightforce ~= self.oldFightForce then
        ...
        fightLabel:runAction(cc.DynamicNumberTo:create('Label', 1, fightforce, function()
            ...
            fightLabel:runAction(cc.Sequence:create(cc.DelayTime:create(0.5),cc.CallFunc:create(function()
                ...
                fightLabel:runAction(cc.Sequence:create(cc.DelayTime:create(0.3),cc.CallFunc:create(function()
                	local fightnumStr = GlobalApi:formatFightNum(fightforce)
                	fightLabel:setString(fightnumStr)
...
function UserObj:getSignByType(stype)
	...
	elseif stype == 'boat' then
		...
		return report or rescopy or guard or self:getUnlimitedStarsShowStatus() or self:getUnlimitedShasShowStatus() or self:getUnlimitedBossBoxShowStatus()
    ...
    elseif stype == 'tavern' or stype == 'pub' then
        --1ban
        --return self:judgeTavenFreeState() or self:judgeTavenTenState()
        --2ban
        return self:judgeTavenFreeState() or self:judgeTavenTenState()
    ...
    elseif stype == 'activitys' then -- 主界面活动按钮
        return self:getActivitySignShowStatus() or self:getActivityLevelGiftShowStatus() or self:getActivityLimitSeckillShowStatus() or self:getActivityTavernRecruitLevelShowStatus() or self:getActivitySaleNewShowStatus() or self:getActivityDayChallengeShowStatus() or self:getActivityPetitionShowStatus()
    ...
    elseif stype == 'sale' then -- 买买买
        return self:getActivityWeekShowStatus() or self:getActivityPrivilegeShowStatus() or self:getActivityGrowFundShowStatus() or self:getActivitLimitBuyShowStatus()
...
function UserObj:getLegionShowStatus()
    ...
    local fighted = MapData:getCurProgress()
...
function UserObj:judgeIsHasRoleOrSpecialFate(id)
    ...
    if roleObj then
    	...
    	local _,heroCombatConf = GlobalApi:getHeroConf(tonumber(id))
        local innateGroupId = heroCombatConf['innateGroup']
...
function UserObj:getLegionTrialShowStatus()
    local isOpen,_,cityId,level = GlobalApi:getOpenInfo_new("trial")
    if not isOpen then
        return false
    end
...
function UserObj:getLegionTrialAdventureShowStatus()
    ...
    local judge 		= false
    local adventure 	= self.trial.adventure

    local nowTime 		= GlobalData:getServerTime()

    for k,v in pairs(adventure) do
        if nowTime < v.time and v.type == 2 then
...
function UserObj:getLegionTrialAchievementShowStatus()
    ...
    local conf 			= GameData:getConfData('trialachievement')

    local weekScore 	= self.trial.week_score or 0
    local joinLv 		= self.trial.join_level

    local baseConf 		= baseConf[LegionTrialMgr:calcTrialLv(joinLv)]
    ...
    for _, v in pairs (arr) do
        if type(v) == 'table' then
			...
			local needScore 		= v['needIntegral']

			if not checkGet( aId ) and weekScore >= needScore then
				isShow = true
				break
			end
	    end
...
function UserObj:getActivityStatus(stype)
    ...
    elseif conf.type == 2 then -- 等级活动
    	local time = 0
    	if self.activity and self.activity[stype] and self.activity[stype]['open_day'] then
    		time = self.activity[stype]['open_day']
    	end
    	if time ~= 0 then
...
function UserObj:setDefaultRefineItem()

	local itemdat = GameData:getConfData('item')
    for k,v in pairs(itemdat) do
        if tostring(v.useType) == 'partexp' then
            table.insert(self.defaultRefineItem,v)
        end
    end
end

function UserObj:getDefaultRefineItem()
	return self.defaultRefineItem
end

-- 获取挖矿剩余次数
function UserObj:getDiggingLeftTimes()
	return self.digmine_times
end

-- 设置挖矿剩余次数
function UserObj:setDiggingLeftTimes( times_ )
	self.digmine_times = times_
end

function UserObj:setTrial( obj_ )
	self.trial = obj_
end

function UserObj:getTrial()
	return self.trial
end
		         
function UserObj:setHookBoss(hookBoss)
	self.hookBoss = hookBoss or {}
end

function UserObj:updateHookBoss(hookBoss)
	self.hookBoss = hookBoss or {}
end

function UserObj:getHookBoss()
	return self.hookBoss
end
