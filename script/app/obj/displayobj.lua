...
function DisplayObj:ctor(data)
    ...
    self.timetype = nil
    self.sid = nil 
	if data[1] == "equip" then --EquipObj:ctor(sid, obj)
		if type(data[3]) ~= 'table' then -- 读表
			self.baseinfo = GameData:getConfData("equipconf")[tonumber(data[2])]
			self.num = math.abs(data[4])	--数量
			self.obj = ClassEquipObj.new(tonumber(data[2]),nil,data[3])
		else -- 服务器发过来的
			self.baseinfo = GameData:getConfData("equipconf")[tonumber(data[3]["id"])]
			self.num = 1
			self.obj = ClassEquipObj.new(tonumber(data[2]), data[3])
		end
		self.frame = ALPHA
	...
	elseif data[1] == "card" then
		self.baseinfo,self.combatInfo,self.modelInfo = GlobalApi:getHeroConf(tonumber(data[2]))
		self.baseinfo.icon = self.modelInfo.headIcon
	...
	elseif data[1] == "group" then
		local config = GameData:getConfData("itemgroupbase")
		self.baseinfo = config[tonumber(data[2])]
...
function DisplayObj:getNameOutlineColor()
	return COLOROUTLINE_QUALITY[self.baseinfo.quality or 5]
end
...
function DisplayObj:qualityLight()
	return QUALITY_LIGHT[self.baseinfo.quality or 5]
end
...
function DisplayObj:getCornerImg()
	return ''
end

function DisplayObj:getCornerTx()
	return ''
end

function DisplayObj:getCornerRTx()
	return ''
end
...
function DisplayObj:getIcon()
	...
	elseif self.type == 'card' then
		return "uires/icon/hero/" .. self.modelInfo.headIcon
	...
	elseif self.type == "group" then
		return "uires/icon/group/" .. self.baseinfo.icon
	else
...
--装备品级
function DisplayObj:getGrade()
	if self.type == 'equip' then
		return self.obj:getGrade()
	else
		return 0
	end
end
