...
function ItemObj:ctor(id,num,isNew)
	...
	if self.conf == nil then
		logger_error('[ERROR]: item can`t find, item id ==', id)
	end
	self.num = num or 1
	self.isNew = isNew
end
...
function ItemObj:getBgImg()
	return COLOR_FRAME_TYPE[self.conf['quality']]
end
...
-- 背包显示
function ItemObj:getBagTab()
	return self.conf['bagTab']
end

--背包类型
function ItemObj:getBagType()
	return self.conf['bagType']
end
