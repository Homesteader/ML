...
local ClassPartObj = require('script/app/obj/partobj')
...
--[[							--小兵结构
    ...
    for i=1,#attconf do
        ...
        -- 觉醒属性
        self.awakeAttr[i] = 0
        self.awakeAttrPercent[i] = 0
	end

    self.partMap = {}
    if obj.part then
        for k, v in pairs(obj.part) do 
            local partId = tonumber(k)
            self.partMap[partId] = ClassPartObj.new(partId,v)
        end
    end
end
...
--部位
function RoleObj:getPartByIndex(index)
	return self.partMap[index]
end
...
--大本营搜寻派遣中
function RoleObj:setDispatchFlag(flag)
	self.isDispathing = flag or false
end

function RoleObj:getDispatchFlag()
	self.isDispathing = self.isDispathing or false
	return self.isDispathing
end
			...
			if  not belongObj then
				if equipObj then
					...
					if isOk then
						...
						equiatt = clone(obj:getAllAttr())
						for i=1,obj:getMaxGemNum() do
							local gemObj = clone(equipObj:getGems()[i])
							if gemObj then
...
function RoleObj:isHavebetterEquipOutSide()
	...
	for x=1,6 do
		local ishave = false
		local equips = {}
		local equipObj = clone(self.equipMap[x])
		local fightforce = 0
		if equipObj then
			fightforce = equipObj:getFightForce()
		end
		if BagData:getEquipNumByPos(x) == 0 then
			...
			local equipMap = clone(BagData:getEquipMapByType(x))
			local equiatt = {}
			for k, v in pairs(equipMap) do
				if equipObj then
					local att = {}	
					...
					if godId == 0 and godId1 ~= 0 then
						local godAttr = clone(equipObj:getGodAttr())
...
function RoleObj:isSoldierCanLvUp()
	...
	for i=1,SOLDIER_EQUIP_COUNT do
...
function RoleObj:getAutoExchangeEquips()
	...
	for i=1,6 do
		...
		local equipObj1 = clone(self.equipMap[i])
		if canexchange then
			local equips = {}
...
function RoleObj:isTupo()
    ...
    if award2 and award2:getOwnNum() < award2:getNum() then
    	iscantupo  = false
    end
...
-- 获取武将碎片数
function RoleObj:getFragmentNum()
	local fragmentnum = 0
	local fragmentobj =BagData:getFragmentById(self.hid)
	if fragmentobj ~= nil then
		fragmentnum =fragmentobj:getNum()
...
function RoleObj:isJunZhu()
	if self.hid > 0 then
		local _,heroCombatConf = GlobalApi:getHeroConf(self.hid)
		if heroCombatConf then
			return heroCombatConf.camp == 5
		else
			return false
		end
	else
...
function RoleObj:isJunZhuforSort()
	...
	if self.hid > 0 then
		local _,heroCombatConf = GlobalApi:getHeroConf(self.hid)
		if heroCombatConf and heroCombatConf.camp == 5 then
			rv = 1
		end
	end
	return rv 
...
function RoleObj:getSoldierdressAtts(lv,star)
	if self.baseConf then        
		local soldiertype = tonumber(self.baseConf['soldierId'])
		lv = lv or self.soldier.level 
		star = star or self.soldier.star
        if lv >= MAXSOLDIERLV then
            return nil
        end

        local soldierLvCfg = GameData:getConfData('soldierlevel')[soldiertype][lv][star]
        local num = self:getSoldierdressNum()
        local atts = {}
        local allAtts = {}
        for i=1,SOLDIER_EQUIP_COUNT do 				--i：代表套装数量
        	local attrId = soldierLvCfg["dress"..i][1]
        	local attrValue = soldierLvCfg["dress"..i][2]
            if attrId and attrValue and attrId > 0 then
                if i <= num then
                    atts[i] = {}
                    atts[i].att1 = attrId
                    atts[i].value1 = attrValue
                end
                allAtts[i] = {}
                allAtts[i].att1 = attrId
                allAtts[i].value1 = attrValue
            end
        end
        return atts,allAtts
    end
...
--小兵等级
function RoleObj:setSoldierLv(soldierLv)
	self.soldier.level = soldierLv
end
...
--小兵星级
function RoleObj:setSoldierStar(star)
	self.soldier.star  = star
end

function RoleObj:getSoldierStar()
	return self.soldier.star
end
...
function RoleObj:getUrl()
	local _,_,heroModelConf = GlobalApi:getHeroConf(tonumber(self.hid))
	return heroModelConf.modelUrl
end

--计算缘分
function RoleObj:calFate()
    if self.combatConf == nil then
        return
    end

	local fatearr = self.combatConf['fateGroup']
	...
	for i=1,#fatearr do
		...
		if tonumber(fatearr[i]) > 0 then
			...
			if tonumber(fatearr[i]) > 1000 then -- 主角缘分
				...
				for j = 1, 5 do
					fatehidarr[j] = fateidconf['hid'..j]
					if fatehidarr[j] == self.hid then 		--排除自己
						fatehidarr[j] = 0 
					end
				end
			end
...
function RoleObj:calbaseAtt()
	local heroBaseConf,heroCombatConf,heroModelConf = GlobalApi:getHeroConf(tonumber(self.hid))
	
	self.baseConf = heroBaseConf
	self.combatConf = heroCombatConf
	self.modelConf = heroModelConf

 	if self.baseConf then
 		local rebornType = self.combatConf['rebornType']
 		local rebornconf = GameData:getConfData('reborn')[rebornType][self.talent]
		local starconf = GameData:getConfData('heroqualityattr')[self.heroQuality][self.combatConf['ability']]

		self.quality = self.combatConf['quality']
		...
		attbase[9] = self.combatConf['baseSpeed']
		attbase[10] = self.combatConf['attackSpeed']
...
--获取羁绊属性
function RoleObj:getObjfetterAtt()

	local fetterAtt = {}
	if self.baseConf then

		local innategroupId = self.combatConf.innateGroup
		...
		if teamheroID == 0 then
			return fetterAtt
		end
		local teamHeroObj = RoleData:getRoleById(teamheroID)
		if not teamHeroObj then
			return fetterAtt
		end
		local mineTuPoLv,teamTuPoLv = self:getTalent(),teamHeroObj:getTalent()
		local teamvaluegroupTab = innategroupCfg.teamvaluegroup
		local idx = 1
		for i=1,#teamvaluegroupTab do
			local att = {0,0,0,0}
			local needValue = teamvaluegroupTab[i]
			local innateid,innateidValue = innategroupCfg["teamId"..i],innategroupCfg["teamValues"..i] 
			if mineTuPoLv >= needValue and teamTuPoLv >= needValue then
				if innateid < 1000 then
					local innateaddtype = tonumber(innateCfg[innateid].type)
					att[1] = innateCfg[innateid].att1
					att[2] = innateidValue
					local id2 = innateCfg[innateid].att2
					if id2 >0 then
						att[1] = innateCfg[innateid].att2
						att[2] = innateidValue
					end
				end
			end
...
function RoleObj:getTupoactiveAtt()
	...
	if self.baseConf then
		local teamnum = 1
		local innateGroupconf =GameData:getConfData('innategroup')[tonumber(self.combatConf['innateGroup'])] 
...
--小兵升级属性
function RoleObj:getSoldierUpgradeAtt(lv,star,isnext)
	...
	if self.baseConf then
		local soldiertype = tonumber(self.baseConf['soldierId'])
		local conf = GameData:getConfData('soldierlevel') [soldiertype][lv][star]
		for j=1,4 do
...
--小兵真实属性
function RoleObj:getSoldierAtt(lv,star,isnext)
...
function RoleObj:getIcon()
	if self.modelConf then
		return 'uires/icon/hero/' .. self.modelConf['headIcon']
	else
		return ''
	end
end
...
function RoleObj:getBigCardImg()
	if self.modelConf then
		local url = 'uires/icon/card_hero/' .. self.modelConf['bodyIcon']
...
function RoleObj:getBigIcon()
	if self.modelConf then
		return 'uires/icon/big_hero/' .. self.modelConf['bodyIcon']
	else
		return ''
	end
end
...
function RoleObj:getProfessionType()
	return tonumber(self.combatConf['professionType'])
end
--职业图片
...
function RoleObj:getRealQulity()
	if self.hid > 0 and self.combatConf then
		return self.combatConf.quality
	else
		return 1
	end
end
...
function RoleObj:getNameOutlineColor()
	if self.baseConf then
		return COLOROUTLINE_QUALITY[self:getQuality()]
	else
...
function RoleObj:getrebornType()
	local borntype = 1
	if self.combatConf then
		borntype = self.combatConf['rebornType']
	end
	return  borntype
end
--突破基础属性
...
function RoleObj:getInnateGroup()
	local groupId = 0
	if self.combatConf then
		groupId = self.combatConf['innateGroup']
	end
	return groupId
end
--突破配置
...
function RoleObj:getSkillIdTab()
	...
	local skillgroup = self.combatConf['skillGroupId'][1]
	if protype > 0  then
		skillgroup = self.combatConf['skillGroupId'][protype]
	else
		skillgroup = self.combatConf['skillGroupId'][1]
	end
...
function RoleObj:getFateArr(isGetActive)
	...
	for i=1,#self.fateinfoarr do
		...
		local atttab  = fateconf[tonumber(self.fateinfoarr[i].fateid)]['att11']
		local attvalue = fateconf[tonumber(self.fateinfoarr[i].fateid)]['value11']
		atttemp.fname = fname
		...
		local atttab2 = fateconf[tonumber(self.fateinfoarr[i].fateid)]['att12']
		local attvalue2 = fateconf[tonumber(self.fateinfoarr[i].fateid)]['value12']
...
function RoleObj:getSoldierArmArr()
	...
	if self.baseConf then
		local soldiertype = tonumber(self.baseConf['soldierId'])
		release_print('soldiertype = ' .. soldiertype .. ', lv = ' .. self.soldier.level .. ', star = ' .. self.soldier.star )
		if self.soldier.level > MAXSOLDIERLV then
			self.soldier.level = MAXSOLDIERLV
		end
		local conf = soldierlvconf[soldiertype][self.soldier.level][self.soldier.star]
		for i=1,SOLDIER_EQUIP_COUNT do
			local  arr ={}
...
function RoleObj:getUiOffsetY()
	return self.modelConf['uiOffsetY']
end
...
function RoleObj:getConfig(config)
	if config == 'hero' then
		local heroBaseConf = GlobalApi:getHeroConf(self.hid)
		return heroBaseConf
...
function RoleObj:playSound(soundType)
	local _,_,heroModelConf = GlobalApi:getHeroConf(self.hid)
	local audioId = nil
	if soundType == 'sound' then
		if heroModelConf.soundEffect ~= 0 then
			audioId = AudioMgr.playEffect('media/hero/'..heroModelConf.soundEffect..'.mp3', false)
		end
	elseif soundType == 'dead' then
		if heroModelConf.deadEffect ~= 0 then
			audioId = AudioMgr.playEffect('media/hero/'..heroModelConf.deadEffect..'.mp3', false)
		end
	end
...
function RoleObj:getChangeEquipState(customObj)
	...
	return GlobalApi:getChangeEquipState(promote, weapon_illusion, wing_illusion)
end

-- 部位相关接口
function RoleObj:getAwakeAttr()
    
    local awakeAttr = {}
    for i=1,MAXEQUIPNUM do
    	local baseAttr = self:getBaseAwakeAttr(i)
    	for k,v in pairs(baseAttr) do
    		awakeAttr[k] = awakeAttr[k] or 0
    		awakeAttr[k] = awakeAttr[k] + (v or 0)
    	end

    	local titleAwakeAttr = self:getTitleAwakeAttr(i)
    	for k,v in pairs(titleAwakeAttr) do
    		awakeAttr[k] = awakeAttr[k] or 0
    		awakeAttr[k] = awakeAttr[k] + (v or 0)
    	end
    end

    local awakeMasterAttr = self:getAwakeMasterAttr()
    for k,v in pairs(awakeMasterAttr) do
		awakeAttr[k] = awakeAttr[k] or 0
    	awakeAttr[k] = awakeAttr[k] + (v or 0)
	end
...
--觉醒基本属性
function RoleObj:getBaseAwakeAttr(part_pos)

	local attr = {}
    local partBaseConf = GameData:getConfData("partbase")
    local partAwakeConf = GameData:getConfData("partawake")
    local maxAttrId = partBaseConf[part_pos].maxAtt 			--极限觉醒
    local maxAttrType = partBaseConf[part_pos].maxValType 		--极限觉醒属性类型(1:常规加成 2:百分比)
    local awakelv = self.partMap[part_pos]:getAwakeLv()
    local maxAwake = self.partMap[part_pos]:getMaxAwakeLv()
    local awakeCfg = partAwakeConf[part_pos][awakelv]
    if not awakeCfg then
    	return attr
    end

    local equipObj = self.equipMap[part_pos]
    if equipObj then
    	local addPercentValue = awakeCfg.addEquipMainAtt	--装备主属性百分比加成
    	local equipMainAttr = equipObj:getMainAttribute();
    	local normalAttrId = equipMainAttr.attrType
    	local normalValue = equipMainAttr.value
    	attr[normalAttrId] = attr[normalAttrId] or 0
        attr[normalAttrId] = attr[normalAttrId] + math.floor(normalValue*(addPercentValue/100))
    end
    ...
    local maxAttrValue = awakeCfg.maxVal
    attr[maxAttrId] = attr[maxAttrId] or 0
    attr[maxAttrId] = attr[maxAttrId] + maxAttrValue
 	
 	return attr
end

-- 刷新觉醒大师属性
function RoleObj:getAwakeMasterAttr()

	local attr = {}
	local awakeMasterLv = self:getAwakeMasterLevel()
	local awakeMasterConf = GameData:getConfData("partmasterawake")
    local newMasterConf = awakeMasterConf[awakeMasterLv]
    if newMasterConf  then
        local attr1 = newMasterConf.attribute[1]
        local attr2 = newMasterConf.attribute[2]
        local attr1Type,attr1Value = GlobalApi:getAttrTypeAndValue(attr1)
        local attr2Type,attr2Value = GlobalApi:getAttrTypeAndValue(attr2)

        -- 默认加固定值
        attr[attr1Type] = attr[attr1Type] or 0
        attr[attr1Type] = attr[attr1Type] + attr1Value
        attr[attr2Type] = attr[attr2Type] or 0
        attr[attr2Type] = attr[attr2Type] + attr2Value
    end
...
-- 刷新头衔激活的觉醒属性
function RoleObj:getTitleAwakeAttr(part_pos)

	local attr = {}
    local partBaseConf = GameData:getConfData("partbase")
    local partTitleConf = GameData:getConfData("parttitleactivate")
    local maxAttrId = partBaseConf[part_pos].maxAtt
    local maxAttrType = partBaseConf[part_pos].maxValType
    local heroQuality = self:getHeroQuality()
    local conf = partTitleConf[part_pos][heroQuality]
    if conf then
    	local awakelv = self.partMap[part_pos]:getAwakeLv()
	    if awakelv >= conf.needAwakeLevel then
	    	local equipObj = self.equipMap[part_pos]
	    	if equipObj then
	    		local equipMainAttr = equipObj:getMainAttribute();
	        	local addPercentValue = conf.addEquipMainAtt
	        	local normalAttrId = equipMainAttr.attrType
	        	local normalValue = equipMainAttr.value
...
-- 获取觉醒大师等级
function RoleObj:getAwakeMasterLevel()

    local awakeLevelParts = {}
    for i = 1, 10 do
        awakeLevelParts[i] = 0
    end

    for i = 1, MAXEQUIPNUM do
        local partObj = self:getPartByIndex(i)
        local partAwakeLevel = partObj:getAwakeLv()
        if partAwakeLevel > 0 then
            for j = partAwakeLevel, 1, -1 do
                awakeLevelParts[j] = awakeLevelParts[j] + 1
            end
        end
    end
    ...
    local partAwakeMasterConf = GameData:getConfData("partmasterawake")
    for i = #partAwakeMasterConf, 1, -1 do
        local needLevel = partAwakeMasterConf[i].needAwakeLevel
        local needNum = partAwakeMasterConf[i].needAwakeNum

        if awakeLevelParts[needLevel] >= needNum then
            masterLevel = i
            break
        end
    end
...
-- 获取部位镶嵌属性
function RoleObj:getEmbedAttr()
    
    local attr = {}
    for partPos = 1,MAXEQUIPNUM do
    	local attrbase = self:getEmbedGemsAttrByPos(partPos)
    	for k,v in pairs(attrbase) do
    		attr[k] = attr[k] or 0
    		attr[k] = attr[k] + (v or 0)
    	end
    end
    return attr
end

function RoleObj:getEmbedGemsAttrByPos(part_pos)

	local attr = {}
    -- 获取部位镶嵌等级
    local partObj = self:getPartByIndex(part_pos)
    local embedLevel = partObj:getPartEmbedLevel()

    -- 装备主属性增加的百分比
    if embedLevel > 0 then
        local embedConf = GameData:getConfData("partembed")
        local attrValue = embedConf[embedLevel].addEquipMainAtt
   		local equipObj = self:getEquipByIndex(part_pos)
	    if equipObj then
			local equipMainAttr = equipObj:getMainAttribute();
	    	local normalAttrId = equipMainAttr.attrType
	    	local normalValue = equipMainAttr.value
			...
			attr[normalAttrId] = attr[normalAttrId] or 0
	        attr[normalAttrId] = attr[normalAttrId] + math.floor(normalValue*(attrValue/100))
		end
    end

    --获取镶嵌大师等级
    local embedMasterConf = GameData:getConfData("partmasterembed")
    local embedMsterLv = self:getEmbedMasterLevel()
    local conf = embedMasterConf[embedMsterLv]
    if conf then
        local attr1 = newConf.attribute[1]
        local attr2 = newConf.attribute[2]
        local attr1Type,attr1Value = GlobalApi:getAttrTypeAndValue(attr1)
        local attr2Type,attr2Value = GlobalApi:getAttrTypeAndValue(attr2)
    ...
    for k,v in pairs(gems) do
    	local attrId = v:getAttrId()
    	local attrValue = v:getValue()
    	attr[attrId] = attr[attrId] or 0
...
-- 获取镶嵌大师等级
function RoleObj:getEmbedMasterLevel()
    local embedMasterLevel = 0
    local partEmbedMasterConf = GameData:getConfData("partmasterembed")
    for i = #partEmbedMasterConf, 1, -1 do
        -- 一个一个部位检查，看是否都满足条件
        local needLevel = partEmbedMasterConf[i].needGemLevel
        local needNum = partEmbedMasterConf[i].needGemNum

        local conditionFit = true
        for j = 1, 6 do
            local hasNum = 0
            local partObj = self:getPartByIndex(j)
            local gems = partObj:getGems(j)
            for k = 1, PART_GEMS_COUNT do
                if gems[k]  then
                    if gems[k]:getLevel() >= needLevel then
                        hasNum = hasNum + 1
                    end
                end
            end
        ...
        -- 找到满足条件的等级了
        if conditionFit == true then
            embedMasterLevel = i
            break
        end
    end

    return embedMasterLevel
end

--获取强化大师等级
function RoleObj:getStrengthenMasterLv()

	local masterLv = 0
	local strengthenCfg = GameData:getConfData("equipmasterupgrade")
	for i=1,#strengthenCfg do
		local needEquip = strengthenCfg[i].needUpgradeNum
		local neddStrengthenLv = strengthenCfg[i].needUpgradeLevel

		local fitCont = 0
		for j=1,MAXEQUIPNUM do
			local equipObj = self:getEquipByIndex(j)
			if equipObj then
				local strengthenLv = equipObj:getStrengthenLv()
				if strengthenLv >= neddStrengthenLv then
					fitCont = fitCont + 1
				end
			end
		end

		if fitCont >= needEquip then
			masterLv = i
		else
			break
		end
...
--获取精炼大师等级
function RoleObj:getRefineMasterLv()

	local masterLv = 0
	local refineMasterCfg = GameData:getConfData("equipmasterrefine")
	for i=1,#refineMasterCfg do
		local needEquip = refineMasterCfg[i].needRefineNum
		local neddRefineLv = refineMasterCfg[i].needRefineLevel
